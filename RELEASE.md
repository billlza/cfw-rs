...
> The release has one candidate identity: build 40066. Build 40030 is retired
...
> A passing policy or source check alone does not consume build 40066. Its
...
for retiring later candidates. Preserve 40066 while repairing evidence tooling.
...
Recreate the fixed `target/release-worktrees/40066` path at the verified new
commit, create an empty `target` with mode `0700`, and run
`scripts/authorize_release_worktree.sh 40066` from the main checkout before
...
40066, not candidate retirement or a new build allocation.
...
- `target/candidates/0.4.0/ga/40066/signed/Clash for Mac.app` as the signed binary root;
- `target/candidates/0.4.0/ga/40066/stage-inputs/publication` as the final evidence root.
...
```bash
publication_artifact_repository="$PWD/target/release-worktrees/40066"
publication_ga="$publication_artifact_repository/target/candidates/0.4.0/ga/40066"
...
### Single-GA 40066 release sequence
...
[`ga-build-40048-retirement.md`](docs/release/ga-build-40048-retirement.md).
Build 40066 is the sole `active_ga` identity. Local installation and smoke
...
supported 40066 transaction without changing application or nested-code
...
3. capture the hosted run through the fixed public GitHub API after freeze.
   ...
   scripts/run_sealed_evidence_manifest.sh collect-ci-lanes \
     --artifact-repository /absolute/operator/target/release-worktrees/40066 \
     --output /absolute/operator/target/release-worktrees/40066/target/candidates/0.4.0/ga/40066/stage-inputs/local-ci-lanes.json \
     --journal /absolute/private/history/local-ci-40066
   ```
...
4. after a signing interruption, inspect the retained attempt through the
   ...
   ```bash
   CFW_BUILD_NUMBER=40066 NOTARY_PROFILE=clashformac-notary \
   ...
   40066, and allocating a successor. Never create fresh timestamped signature
...
5. regenerate and legally review the fixed publication source/SBOM closure,
   ...
   fixed `target/release-worktrees/40066` artifact checkout. These operations
...
7. after the atomic journal export verifies, run the fixed GA runtime collector.
   ...
   contained app equals the installed 40066 tree, derives all twelve required
...
build 40066. No updater key, Apple notarization key, local private key, or older
...
`target/candidates/0.4.0/ga/40066/packages/updater/vVERSION/`. The DMG, accepted result,
...
`target/candidates/0.4.0/ga/40066/packages/dmg/vVERSION/`. The canonical seals bind exact
...
The atomic
`ga/40066/packages/distribution/vVERSION/distribution-set.seal.json` joins the
