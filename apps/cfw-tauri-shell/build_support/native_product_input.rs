...
const PREVIEW_PRE_SIGN_RELATIVE_ROOT: &str = "preview-preflight/50015/native-products";
const PREVIEW_BUILD_NUMBER: &str = "50015";
...
mod preview_tests {
    ...
    fn preview_path_build_and_manifest_are_one_exact_identity() {
        ...
        let resolved = CandidateNativeProducts::resolve(&root, &output, "50015").unwrap();
        assert_eq!(resolved.context, NativeProductContext::PreviewPreSign);
        assert_eq!(resolved.context.expected_build_number(), "50015");
        assert_eq!(resolved.context.expected_signing_mode(), "pre-sign");
        let metadata = BTreeMap::from([
            ("buildNumber".into(), "50015".into()),
    ...
    fn preview_refuses_other_builds_and_legacy_context_paths() {
        ...
        for relative in [
            ...
            "preview-preflight/50016/native-products",
            "preview/50015/signing-output/signed-native-products",
        ] {
            let wrong = fixture.output("0.5.0", relative);
            assert!(
                CandidateNativeProducts::resolve(&root, &wrong, "50015").is_err(),
                "{relative}"
            );
        }
        let old_root = fixture.root("0.4.0");
        let wrong = fixture.output("0.4.0", PREVIEW_PRE_SIGN_RELATIVE_ROOT);
        assert!(CandidateNativeProducts::resolve(&old_root, &wrong, "50015").is_err());
        for (relative, build, expected) in [
            ...
            assert!(CandidateNativeProducts::resolve(&old_root, &old, "50015").is_err());
        }
    }
    ...
    fn unsigned_preview_context_is_disjoint_from_every_production_root_and_mode() {
        ...
        for (version, relative, build) in [
            ...
            ("0.5.0", PREVIEW_PRE_SIGN_RELATIVE_ROOT, "50015"),
        ] {
    ...
    fn preview_rejects_raw_alias_and_symlinked_roots() {
        ...
        for wrong in [
            ...
            assert!(CandidateNativeProducts::resolve(&root, &wrong, "50015").is_err());
        }
        let raw_root = root
            .to_str()
            .unwrap()
        ...
        assert!(
            CandidateNativeProducts::resolve(Path::new(&raw_root), &raw_output, "50015").is_err()
        );
        let moved = root.with_file_name("moved-preview");
        fs::rename(&root, &moved).unwrap();
        std::os::unix::fs::symlink(&moved, &root).unwrap();
        assert!(CandidateNativeProducts::resolve(&root, &output, "50015").is_err());
    }
}
