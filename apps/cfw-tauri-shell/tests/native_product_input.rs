...
impl CandidateFixture {
    fn new() -> Self {
        ...
        fs::create_dir_all(candidate_root.join("ga-preflight/40060/native-products"))
...
fn exact_candidate_roots_select_one_context() {
    ...
    let ga = CandidateNativeProducts::resolve(
        &fixture.candidate_root,
        &fixture.output("ga-preflight/40060/native-products"),
        "40060",
    )
    ...
    assert_eq!(ga.context.expected_build_number(), "40060");
}
...
fn candidate_root_and_build_number_must_match_exactly() {
    ...
    for (relative, build_number) in [
        ("unsigned/native-products", "40060"),
        ("ga-preflight/40060/native-products", "40000"),
        ("ga-preflight/40060/native-products", "40043"),
        ("ga-preflight/40060/native-products", "40044"),
        ("ga-preflight/40060/native-products", "40045"),
        ("ga-preflight/40060/native-products", "040049"),
        ("ga-preflight/40060/native-products", "0"),
        ("ga-preflight/40060/native-products", "not-a-build"),
        ("ga-preflight/40060/native-products", "9223372036854775808"),
    ] {
...
fn unapproved_or_noncanonical_candidate_paths_are_rejected() {
    let fixture = CandidateFixture::new();
    for declared in [
        fixture.output("validation/40060/native-products"),
        fixture.output("release-build/40060/native-products"),
        ...
        fixture.output("ga/40060/signing-output/signed-native-products"),
        fixture.output("ga-preflight/40060/native-products/extra"),
        format!(
            "{}/ga-preflight//40060/native-products",
            fixture.candidate_root.display()
        ),
        format!(
            "{}/ga-preflight/../ga-preflight/40060/native-products",
            fixture.candidate_root.display()
        ),
        "ga-preflight/40060/native-products".to_string(),
    ] {
        let error = CandidateNativeProducts::resolve(&fixture.candidate_root, &declared, "40060")
...
fn missing_unknown_and_developer_id_metadata_are_rejected() {
    for artifact in ARTIFACTS {
        for invalid in [
            metadata("40060", "developer-id"),
            metadata("40060", ""),
            metadata("40060", "unknown"),
        ...
        let mut missing_build = metadata("40060", "pre-sign");
        ...
        let mut missing_mode = metadata("40060", "pre-sign");
