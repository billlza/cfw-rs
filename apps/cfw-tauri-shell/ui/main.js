...
`)).split(/[\n,]+/).map(t=>t.trim()).filter(Boolean),tun_mode:s.toggles.tunMode,mixin:s.toggles.mixin,mixin_yaml:document.querySelector("[data-mixin-yaml]")?.value??s.mixinYaml??e.mixin_yaml??"",profile_parser_script:document.querySelector("[data-profile-parser-script]")?.value??s.profileParserScript??e.profile_parser_script??"",tray_script:document.querySelector("[data-tray-script]")?.value?.trim()??s.trayScript??e.tray_script??"",child_process_command:document.querySelector("[data-child-process-command]")?.value?.trim()??s.childProcessCommand??e.child_process_command??"",allow_lan:s.toggles.allowLan,enable_ipv6:s.toggles.enableIpv6,launch_at_login:s.toggles.startAtLogin,silent_start:s.toggles.silentStart,break_connections_on_proxy_change:s.toggles.breakOnProxyChange,show_tray_proxy_delay_indicator:s.toggles.proxyDelayIndicator,usePacScript:s.toggles.usePacScript,pacScript:document.querySelector("[data-pac-script]")?.value??s.pacScript??e.pacScript??e.pac_script??"",theme:document.querySelector("[data-theme-setting]")?.value??e.theme??"light",font_family:document.querySelector("[data-font-family]")?.value?.trim()??e.font_family??"","interface-name":document.querySelector("[data-outbound-interface]")?.value?.trim()??e["interface-name"]??e.interfaceName??"",randomMixedPort:document.querySelector("[data-random-mixed-port]")?.checked??!!(e.randomMixedPort??e.random_mixed_port),delayTestUrl:document.querySelector("[data-delay-test-url]")?.value?.trim()||e.delayTestUrl||e.delay_test_url||"http://www.gstatic.com/generate_204",coreKind:document.querySelector("[data-core-kind]")?.value||e.coreKind||e.core_kind||"clash_rs",core_kind:document.querySelector("[data-core-kind]")?.value||e.coreKind||e.core_kind||"clash_rs"}}function Je(e){let t=e.theme==="dark"?"dark":"light";document.documentElement.dataset.theme=t;let a=String(e.font_family??e.fontFamily??"").trim();document.documentElement.style.setProperty("--sans",a?`"${a}", "Avenir Next", "SF Pro Text", "Helvetica Neue", sans-serif`:'"Avenir Next", "SF Pro Text", "Helvetica Neue", sans-serif')}function Ze(e){if(!e)return;let t=e.config??{},a=t.mode?t.mode[0].toUpperCase()+t.mode.slice(1).toLowerCase():null;["Global","Rule","Direct","Script"].includes(a)&&(s.mode=a);let o=t["allow-lan"]??t.allow_lan,r=t["mixed-port"]??t.mixed_port;typeof o=="boolean"&&(s.toggles.allowLan=o),typeof t.ipv6=="boolean"&&(s.toggles.enableIpv6=t.ipv6),(t["log-level"]||t.log_level)&&(s.logLevel=t["log-level"]??t.log_level),typeof r=="number"&&(s.settingsSnapshot.settings.mixed_port=r);let n=new Map((e.proxies?.proxies??[]).map(u=>[u.name,u])),l=e.proxies?.groups??[],d=new Map(l.map(u=>[u.name,u]));s.proxyGroups=l.map(u=>({name:u.name,type:u.kind,now:u.now??u.options?.[0]??"DIRECT",options:(u.options??[]).map(h=>{let f=n.get(h),m=d.get(h);return{name:h,delay:Te(f?.history??m?.history??[]),dead:!1,kind:f?.kind??f?.type??m?.kind??u.kind??"Proxy",udp:f?.udp??null}})})),Be(e.connections),s.controllerStatus="controller live"}function Be(e){if(!e)return;let t=Date.now(),a=s.connectionStream.at?Math.max(.25,(t-s.connectionStream.at)/1e3):0,o=e.upload??e.uploadTotal??e.upload_total??0,r=e.download??e.downloadTotal??e.download_total??0,n=s.connectionStream.rows??new Map,l=e.connections??[];s.connections=l.map(d=>{let u=d.metadata??{},h=u.host||u.destinationIP||u.destination_ip||"unknown",f=d.rulePayload??d.rule_payload,m=[d.rule,f].filter(Boolean).join(","),P=n.get(d.id),C=d.upload??0,q=d.download??0,R=P&&a?Math.max(0,(C-P.upload)/a):0,U=P&&a?Math.max(0,(q-P.download)/a):0;return{id:d.id,host:h,rule:m||"MATCH",chains:d.chains??[],upload:S(C),download:S(q),uploadBytes:C,downloadBytes:q,uploadSpeedBytes:R,downloadSpeedBytes:U,speed:`${S(R)}/s up \xB7 ${S(U)}/s down`,age:d.start?d.start.slice(11,19):"live",start:d.start,metadata:u}}),a&&(s.traffic.upload=Math.max(0,(o-s.connectionStream.uploadTotal)/a/1024/1024),s.traffic.download=Math.max(0,(r-s.connectionStream.downloadTotal)/a/1024/1024)),s.connectionStream={at:t,uploadTotal:o,downloadTotal:r,rows:new Map(l.map(d=>[d.id,{upload:d.upload,download:d.download}]))},s.controllerStatus="controller live stream"}function et(e){if(!e)return!1;let t=e.proxy_providers??[],a=e.rule_providers??[];return s.providers=t.map(o=>({name:o.name,type:o.kind,vehicle:o.vehicle_type,updated:o.updated_at??"unknown",health:o.extra?.healthCheck?.lastResult??o.extra?.healthcheck?.lastResult??"Unknown",proxies:o.proxies?.length??0})),s.ruleProviders=a.map(o=>({name:o.name,type:o.kind,behavior:o.behavior??o.vehicle_type,updated:o.updated_at??"unknown",rules:o.rules?.length??0})),!0}function tt(){let e=de(s.logSearch);return s.logs.filter(t=>{let a=s.logFilter==="all"||t.level===s.logFilter,o=[t.time,t.level,t.source,t.message,...(t.fields??[]).map(n=>`${n.key}=${n.value}`)].join(" "),r=!s.logSearch||(e?e.test(o):o.toLowerCase().includes(s.logSearch.toLowerCase()));return a&&r}).slice(0,ie)}function qe(){let e=de(s.connectionSearch),t=s.connections.filter(r=>{let n=r.metadata??{},l=[r.host,r.rule,...r.chains??[],n.processPath,n.process_path,n.sourceIP,n.source_ip,n.destinationIP,n.destination_ip,n.network,n.type].filter(Boolean).join(" ");return!s.connectionSearch||(e?e.test(l):l.toLowerCase().includes(s.connectionSearch.toLowerCase()))}),a={host:r=>r.host,speed:r=>r.uploadSpeedBytes+r.downloadSpeedBytes,upload:r=>r.uploadBytes,download:r=>r.downloadBytes,age:r=>Date.parse(r.start||"")||0},o=a[s.connectionSort]??a.age;return[...t].sort((r,n)=>{let l=o(r),d=o(n),u=typeof l=="string"?l.localeCompare(String(d)):l-d;return s.connectionSortDesc?-u:u}).slice(0,Ce)}function at(){let e=de(s.ruleSearch);return s.rules.filter(t=>{let a=[t.index,t.type,t.payload,t.proxy,t.hits].filter(Boolean).join(" ");return!s.ruleSearch||(e?e.test(a):a.toLowerCase().includes(s.ruleSearch.toLowerCase()))}).slice(0,2e3)}function st(){let e=document.getElementById("nav");e.innerHTML=s.payload.pages.filter(t=>ke.has(t.id)).map((t,a)=>`
  ...
  `}function he(e){!e||typeof e!="object"||(s.updateInfo={available:!!e.available,current:e.current??s.payload?.product?.version,version:e.version??null,notes:e.notes??null,error:e.error??null})}async function He(e=!1){if(s.kernelCompare&&!e)return s.kernelCompare;try{return s.kernelCompare=await p("kernel_compare_report"),s.kernelCompare}catch(t){return i("warning","bench",`Kernel compare unavailable: ${t.message??String(t)}`),null}}async function ze(e){he(e),D({autoCheck:!0,checking:!1,result:e})}function D(e={}){let t=s.payload?.product??T.product,a=e.result?{available:!!e.result.available,current:e.result.current??t.version,version:e.result.version??null,notes:e.result.notes??null,error:e.result.error??null,date:e.result.date??null}:s.updateInfo;s.glassDialog={kind:"product-about",payload:{checking:!!e.checking,autoCheck:!!e.autoCheck,update:a}},_()}function nt(e){if(e?.checking)return"Checking for updates\u2026";let t=e?.update;return t?.error?`Update check failed: ${t.error}`:t?.available&&t?.version?`Update available: v${t.version}`:t&&t.available===!1?`You\u2019re up to date (v${t.current??s.payload?.product?.version??"\u2014"})`:"Check GitHub releases for new builds."}function Oe(){let e=s.settingsSnapshot?.settings??y.settings,a=(s.payload.product??T.product).version??"0.3.3",o=s.updateInfo,r=o?.available&&o?.version?`<button type="button" class="cfw-update-badge" data-action="check-for-updates" title="Update available \u2014 click to download">\u2192 v${c(String(o.version))}</button>`:"",n=s.coreStatus??I,l=n.state==="Running",d=e.external_controller_host??"127.0.0.1",u=e.external_controller_port??9090,h=e.coreKind??e.core_kind??"clash_rs",f=s.controllerVersion?.version??(l?"Connected":n.state),m=l?"cfw-status-dot on":"cfw-status-dot",P=s.logLevel??e.logLevel??e["log-level"]??"info",C=ee(s.serviceModeStatus),q=s.serviceModeStatus==="Enabled",R=!!(e.randomMixedPort??e.random_mixed_port),U=Fe(e),J=ot(s.serviceModeStatus);return`
    <div class="cfw-general-view">
      <section class="cfw-header">
        <div class="cfw-app-mark">${Ue()}</div>
        <div class="cfw-title">
      ...
      <section class="cfw-content">
          ...
          <div class="cfw-row-right">
            <select class="cfw-select" data-log-level>
              ${["info","warning","error","debug","trace","silent"].map(j=>`
                <option value="${j}" ${P===j?"selected":""}>${j}</option>
              `).join("")}
            </select>
          </div>
        </div>
        ...
        <div class="cfw-row">
          ...
          <div class="cfw-row-right">
            <button type="button" class="cfw-text-button core-version-link" data-action="open-controller-dashboard" title="Open controller dashboard">
              <i class="${m}"></i>
              <span>${c(String(h))} \xB7 ${c(f)}${l?` (${c(String(u))})`:""}</span>
            </button>
        ...
        <div class="cfw-row">
          ...
          <div class="cfw-row-right">
            <button class="cfw-text-button" data-action="update-geoip-database" title="${s.geoipStatus?.path?c(s.geoipStatus.path):"Download / refresh GeoIP database"}">${c(Ee(s.geoipStatus,s.geoipUpdating))}</button>
          </div>
        </div>
        ...
        <div class="cfw-row">
          <div class="cfw-row-left">
            ...
            <span class="general-icons">
              <span class="general-icon ${J}" title="Service Mode status">${Ne("public")}</span>
            </span>
          </div>
          <div class="cfw-row-right">
        ...
        <div class="cfw-row">
          ...
          <div class="cfw-row-right">
            <span class="cfw-link-value">${Ae(s.tunRuntime?.tun_mode??s.toggles.tunMode,s.serviceModeStatus,s.tunRuntime)}</span>
  ...
  `}function Ge(e){return e==null?"pending":e<=0?"dead":e<80?"fast":e<180?"mid":"slow"}function je(e){return e==null?"Pending":e<=0?"Timeout":`${e} ms`}function De(){if(typeof document<"u"&&document.hidden)return 2;let e=Number(navigator.hardwareConcurrency)||8;return Math.max(4,Math.min(16,e))}function me(){w.delayTestGeneration=(w.delayTestGeneration??0)+1,s.toggles.testingDelays=!1}function it(){let e=document.querySelector("[data-proxy-node-grid]");if(!e)return[];let t=e.getBoundingClientRect(),a=[];return e.querySelectorAll("[data-proxy-node]").forEach(o=>{let r=o.getBoundingClientRect();if(r.bottom>=t.top&&r.top<=t.bottom){let n=o.getAttribute("data-proxy-node");n&&a.push(n)}}),a}function lt(e){let t=new Set(it()),a=[],o=[];for(let r of e)t.has(r)?a.push(r):o.push(r);return a.length?[...a,...o]:e}function ge(e,t){let a=typeof t=="number"&&Number.isFinite(t)?t:0;s.proxyGroups.forEach(o=>{o.options.forEach(r=>{r.name===e&&(r.delay=a,r.dead=a<=0)})})}function Y(e){let t=e?new Set(e):null;document.querySelectorAll("[data-proxy-delay]").forEach(o=>{let r=o.getAttribute("data-proxy-delay");if(!r||t&&!t.has(r))return;let n=null;for(let l of s.proxyGroups){let d=l.options.find(u=>u.name===r);if(d){n=d.delay;break}}o.className=Ge(n),o.textContent=je(n)});let a=document.querySelector('[data-action="delay-test"]');a&&(a.classList.toggle("active",!!s.toggles.testingDelays),a.disabled=!!s.toggles.testingDelays)}function Re(e){e.forEach(t=>{let a=null;for(let o of s.proxyGroups){let r=o.options.find(n=>n.name===t);if(r){a=r;break}}a&&(a.delay===null||a.delay===void 0)&&ge(t,0)}),Y(e)}function ct(e){return String(e).replace(/[^a-z0-9_-]/gi,"-")}function te(e){return["selector","relay"].includes(String(e??"").toLowerCase())}function pe(e){let t='width="18" height="18" viewBox="0 0 24 24" aria-hidden="true"';switch(e){case"scroll":return`<svg ${t} fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/><circle cx="18.5" cy="18.5" r="3.2" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M20.8 20.8L23 23" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>`;case"report":return`<svg ${t} fill="currentColor"><path d="M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM12 17.3c-.72 0-1.3-.58-1.3-1.3s.58-1.3 1.3-1.3 1.3.58 1.3 1.3-.58 1.3-1.3 1.3zm1-4.3h-2V7h2v6z"/></svg>`;case"report-off":return`<svg ${t} fill="currentColor"><path d="M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM12 17.3c-.72 0-1.3-.58-1.3-1.3s.58-1.3 1.3-1.3 1.3.58 1.3 1.3-.58 1.3-1.3 1.3zm1-4.3h-2V7h2v6z" opacity=".38"/></svg>`;case"delay":return`<svg ${t} fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12a10 10 0 0 1 20 0"/><path d="M5 12a7 7 0 0 1 14 0"/><path d="M8.5 12a3.5 3.5 0 0 1 7 0"/><path d="M12 12V7.5" stroke-width="2"/><circle cx="12" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>`;case"eye":return`<svg ${t} fill="currentColor"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>`;case"eye-off":return`<svg ${t} fill="currentColor"><path d="M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.83l2.92 2.92c1.51-1.26 2.7-2.89 3.43-4.75-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C10.74 7.13 11.35 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.31-.78l3.15 3.15.02-.16c0-1.66-1.34-3-3-3l-.17.01z"/></svg>`;default:return""}}function dt(e){return e?e.dead?!0:typeof e.delay=="number"&&e.delay<=0:!1}function pt(){let e=s.proxyFilter.trim().toLowerCase(),t=s.proxyGroups.map(f=>({...f,options:f.options.filter(m=>{let P=!e||f.name.toLowerCase().includes(e)||m.name.toLowerCase().includes(e),C=s.toggles.hideUnavailable&&dt(m)&&f.now!==m.name;return P&&!C})})).filter(f=>f.options.length||f.name.toLowerCase().includes(e)),a=t.find(f=>f.name===s.activeProxyGroup)??t.find(f=>te(f.type)&&/选择|select|proxy|节点/i.test(f.name))??t.find(f=>te(f.type)&&f.name.toUpperCase()!=="GLOBAL")??t.find(f=>te(f.type))??t[0]??null,o=a?te(a.type):!1,r=!!s.toggles.hideUnavailable,n=s.toggles.showProxiesList!==!1,l=s.proxyBlinkNode,d=s.controllerStatus==="controller live"&&s.proxyGroups.length>0,u=s.controllerStatus==="controller live"&&s.proxyGroups.length===0?"Active profile has no proxy groups. Switch to a subscription with nodes.":"Controller unavailable. No live proxy groups are being displayed.";return`
    <div class="proxy-layout">
      ...
      <div class="mode-switch proxy-mode-header" role="group" aria-label="Proxy mode">
        ${["Global","Rule","Direct","Script"].map(f=>`
          <button class="${s.mode===f?"selected":""}" data-mode="${f}">${f} <span>${ut(f)}</span></button>
        `).join("")}
      </div>`:""}
        ...
        ${a?`
          <div class="cfw-proxy-head">
            ...
            <div class="cfw-proxy-tools">
              ...
              <button class="proxy-tool" data-action="scroll-to-selected-proxy" title="Scroll to selected proxy">${pe("scroll")}</button>
              <button class="proxy-tool ${r?"active":""}" data-action="toggle-hide-timed-out" title="Show/Hide timed-out proxies">${pe(r?"report-off":"report")}</button>
              <button class="proxy-tool ${s.toggles.testingDelays?"active":""}" data-action="delay-test" title="Test latency" ${s.toggles.testingDelays?"disabled":""}>${pe("delay")}</button>
              <button class="proxy-tool ${n?"active":""}" data-action="toggle-show-proxies" title="Show/hide proxies">${pe(n?"eye":"eye-off")}</button>
            </div>
          </div>
          <div class="cfw-proxy-content">
            ...
            <div class="cfw-node-grid" data-proxy-node-grid>
              ${a.options.map(f=>`
                <button class="cfw-node-card ${a.now===f.name?"selected":""} ${l===f.name?"blink":""} ${o?"":"readonly"}" data-proxy-node="${c(f.name)}" ${o?`data-group="${c(a.name)}" data-node="${c(f.name)}"`:"disabled"} title="${o?"Select proxy":"This Clash group type is controlled by the core"}">
                  <i></i>
                  <span>
                    <strong>${gt(f.name)}${c(f.name)}</strong>
                    <small>${c(f.kind??"Proxy")} ${f.udp===!1?"":"<em>UDP</em>"}</small>
                  </span>
                  <b class="${Ge(f.delay)}" data-proxy-delay="${c(f.name)}">${je(f.delay)}</b>
                </button>
              `).join("")}
            </div>
            ...
            <aside class="cfw-group-rail">
              ${t.map(f=>`
                <button class="${f.name===a.name?"active":""}" data-proxy-group-tab="${c(f.name)}" title="${c(f.name)}">${c(ft(f.name))}</button>
              `).join("")}
            </aside>
          </div>
        `:`<p class="empty">${c(u)}</p>`}
...
[::]:53`)}</textarea></label>
      ...
      `);else if(a.kind==="product-about"){let n=(s.payload?.product??T.product).version??"0.3.3",l=nt(a.payload),d=a.payload?.update,u=!!(d?.available&&d?.version&&!a.payload?.checking),h=d?.notes?`<p class="product-about-notes">${c(String(d.notes).slice(0,280))}</p>`:"";t.push(`
        <div class="glass-dialog-backdrop" data-glass-dismiss></div>
        <div class="glass-dialog product-about" role="dialog" aria-label="About Clash for Mac">
          <div class="product-about-icon">${Ue()}</div>
...
[::]:53`;try{let l=x(),d=await p("write_settings_snapshot",{settings:{...l,"tun-stack":a,tunStack:a,"tun-auto-route":o,tunAutoRoute:o,"tun-strict-route":r,tunStrictRoute:r,"tun-dns-hijack":n,tunDnsHijack:n}});if(v(d),A(),i("info","tun",`TUN settings saved (stack=${a})`),s.toggles.tunMode)try{await p("reapply_runtime_config"),i("info","tun","Runtime config reapplied (root core respawn when TUN owns core)")}catch(u){i("warning","tun",`Reapply after TUN settings failed: ${u.message??String(u)}`)}}catch(l){i("warning","tun",`TUN settings failed: ${l.message??String(l)}`)}g()})});let e=async t=>{let a=document.querySelector("[data-glass-restore-dns]")?.value??"Empty";try{let o=x(),r=await p("write_settings_snapshot",{settings:{...o,"restore-dns-servers":a,restoreDnsServers:a}});if(v(r),t){let n=await p("apply_restore_dns_servers",{servers:a});i("info","dns",n)}else i("info","dns","Restore DNS list saved");A()}catch(o){i("warning","dns",o.message??String(o))}g()};document.querySelectorAll("[data-glass-restore-dns-save]").forEach(t=>{t.addEventListener("click",()=>e(!1))}),document.querySelectorAll("[data-glass-restore-dns-apply]").forEach(t=>{t.addEventListener("click",()=>e(!0))}),document.querySelectorAll("[data-glass-service-install]").forEach(t=>{t.addEventListener("click",async()=>{try{let a=await p("enable_service_mode");s.serviceModeStatus=a,i("info","service",`Service Mode: ${ee(a)}`)}catch(a){i("warning","service",a.message??String(a))}A(),g()})}),document.querySelectorAll("[data-glass-service-uninstall]").forEach(t=>{t.addEventListener("click",async()=>{try{await p("disable_service_mode"),s.serviceModeStatus=await p("service_mode_status"),i("info","service","Service Mode disabled")}catch(a){i("warning","service",a.message??String(a))}A(),g()})}),document.querySelectorAll("[data-glass-service-login-items]").forEach(t=>{t.addEventListener("click",async()=>{try{await p("open_login_items_settings"),i("info","service","Opened Login Items settings")}catch(a){i("warning","service",a.message??String(a))}})}),document.querySelectorAll("[data-glass-check-update]").forEach(t=>{t.addEventListener("click",async()=>{D({autoCheck:!0,checking:!0,result:s.updateInfo});try{let a=await p("check_for_updates");he(a),i((a?.available,"info"),"updater",a?.available?`Update ${a.version} available`:`Already up to date (${a?.current??"current"})`),D({autoCheck:!0,checking:!1,result:a})}catch(a){i("warning","updater",`Update check failed: ${a.message??String(a)}`),D({autoCheck:!0,checking:!1,result:{available:!1,error:a.message??String(a),current:s.payload?.product?.version}})}})}),document.querySelectorAll("[data-glass-install-update]").forEach(t=>{t.addEventListener("click",async()=>{try{i("info","updater","Downloading and installing update\u2026"),D({autoCheck:!0,checking:!0,result:{...s.updateInfo,available:!0}}),await p("install_available_update")}catch(a){i("warning","updater",`Install failed: ${a.message??String(a)}`),D({autoCheck:!0,checking:!1,result:{...s.updateInfo,error:a.message??String(a)}})}})})}async function We(e,t){let a=s.profiles.find(o=>o.id===t);if(!a)throw new Error(`profile not found: ${t}`);switch(e){case"select":await $e(t);return;case"open-web":{if(!a.homeWeb)throw new Error("no profile-web-page-url for this subscription");await p("open_external_url",{url:a.homeWeb}),i("info","profile",`Opened web page for ${a.name}`);return}case"edit":await B(t,"edit"),i("info","profile",`edit editor opened for ${a.name}`);return;case"proxies":await B(t,"edit","proxies"),i("info","profile",`proxies section editor opened for ${a.name}`);return;case"rules":await B(t,"edit","rules"),i("info","profile",`rules section editor opened for ${a.name}`);return;case"edit-external":await p("open_profile_externally",{id:t}),i("info","profile",`Opened ${a.name} externally`);return;case"update":{if(!a.sourceUrl)throw new Error("local profile has no subscription URL");let o=a.active,r=await p("update_profile",{id:t});await $(),o&&await p("apply_active_profile"),i("info","profile",`${r.name} subscription updated${o?" and config.yaml reapplied":""}`);return}case"reveal":await p("reveal_profile",{id:t}),i("info","profile",`Show in folder: ${a.name}`);return;case"diff":await B(t,"diff"),i("info","profile",`Diff opened for ${a.name}`);return;case"copy":s.glassDialog={kind:"copy",id:t},_();return;case"qrcode":await B(t,"qrcode"),i("info","profile",`QRCode opened for ${a.name}`);return;case"parsers":await B(t,"parsers"),i("info","profile",`Parsers opened for ${a.name}`);return;case"run-script":await B(t,"parsers"),a.active?(await p("apply_active_profile"),i("info","profile",`Run script: reapplied parsers/mixin for active profile ${a.name}`)):i("info","profile",`Run script: parser pipeline runs on apply; select ${a.name} to execute against config.yaml`);return;case"settings":s.glassDialog={kind:"settings",id:t},_();return;case"delete":s.glassDialog={kind:"delete",id:t},_();return;default:throw new Error(`unknown profile menu action: ${e}`)}}function kt(){return`
        ...
        ${s.profiles.length?s.profiles.map(e=>`
          <article class="cfw-profile-card ${e.active?"active":""}" data-profile-card="${c(e.id)}">
            ...
            <div class="profile-card-main">
              <h3>${c(e.name)}</h3>
              <p>${c(e.sourceUrl?ht(e.sourceUrl):"local file")} (${c(e.updated)})</p>
              <div class="profile-usage">
                ${yt(e)}
              </div>
              <div class="profile-progress" title="${e.subscriptionUserinfo?c(e.subscriptionUserinfo):"No subscription usage quota"}"><b style="width:${bt(e).toFixed(1)}%"></b></div>
            </div>
  ...
  `}function Ct(){let e=s.profileInspector;if(!e)return"";let t=e.profile??{},a=`${t.name??e.id} \xB7 ${e.mode}`,o=t.source_url??t.sourceUrl??null,r=t.body?[...t.body.matchAll(/^([A-Za-z0-9_-]+):/gm)].map(l=>l[1]).filter(l=>l.toLowerCase().includes("parser")):[],n="";return e.mode==="diff"?n=`
  ...
  `}function Lt(e){let t=e.metadata?.processPath??e.metadata?.process_path??e.processPath??"";if(!t)return"\u2014";let a=String(t).split(/[/\\]/).filter(Boolean);return a[a.length-1]||String(t)}function Ke(e,t){return`
    <article class="cfw-conn-item${t?" with-process":""}" data-connection-id="${c(e.id)}">
      ...
      ${t?`<div class="conn-process" title="${c(e.metadata?.processPath??e.metadata?.process_path??"")}">${c(Lt(e))}</div>`:""}
  ...
  `}function At(){let e=qe(),t=s.connections.find(n=>n.id===s.connectionDetailId),a=S(s.connectionStream.uploadTotal),o=S(s.connectionStream.downloadTotal),r=s.toggles.showProcess!==!1;return w.connectionRowEls=null,`
  ...
  `}function Et(){let e=document.querySelector("[data-connections-root]"),t=document.querySelector("[data-conn-scroll]");if(!e||!t||s.activePage!=="connections"){g();return}let a=qe(),o=s.toggles.showProcess!==!1,r=e.querySelector("[data-conn-totals]");r&&(r.textContent=`Total: \u2191 ${S(s.connectionStream.uploadTotal)} \u2193 ${S(s.connectionStream.downloadTotal)}`);let n=e.querySelector("[data-conn-close-all]");n&&(n.disabled=!!s.closingAllConnections,n.textContent=s.closingAllConnections?"Closing...":`Close All (${a.length})`),w.connectionRowEls instanceof Map||(w.connectionRowEls=new Map,t.querySelectorAll("[data-connection-id]").forEach(m=>{w.connectionRowEls.set(m.getAttribute("data-connection-id"),m)}));let l=new Set(a.map(m=>m.id));for(let[m,P]of[...w.connectionRowEls.entries()])l.has(m)||(P.remove(),w.connectionRowEls.delete(m));let d=document.createDocumentFragment(),u=!1;a.forEach((m,P)=>{let C=w.connectionRowEls.get(m.id);if(!C){let Z=document.createElement("div");Z.innerHTML=Ke(m,o).trim(),C=Z.firstElementChild,w.connectionRowEls.set(m.id,C),u=!0,d.appendChild(C);return}let q=C.querySelector("[data-conn-up]"),R=C.querySelector("[data-conn-down]"),U=C.querySelector("[data-conn-speed]");q&&(q.textContent=`\u2191 ${m.upload}`),R&&(R.textContent=`\u2193 ${m.download}`),U&&(U.textContent=m.speed??"0 B/s");let J=C.querySelector("[data-close-connection]");if(J){let Z=s.closingConnectionIds.has(m.id);J.disabled=Z,J.textContent=Z?"Closing":"Close"}let j=t.children[P];!u&&j!==C&&t.insertBefore(C,j??null)}),d.childNodes.length&&t.appendChild(d);let h=s.connections.find(m=>m.id===s.connectionDetailId),f=e.querySelector(".modal-backdrop");h&&!f?(e.insertAdjacentHTML("beforeend",Ye(h)),Xe()):!h&&f&&f.remove()}function Tt(){w.connectionsPatchFrame===null&&(w.connectionsPatchFrame=window.requestAnimationFrame(()=>{w.connectionsPatchFrame=null,Et()}))}function Ye(e){let t=e.metadata??{};return`
  ...
  `}function Rt(){let e=s.networkDiagnostics;if(!e)return F("Network Diagnostics",[b("Default Route","Unavailable","Run inside the Tauri app to inspect macOS service order.")]);let t=e.services??[],a=e.recommended_clash_proxy_services??[],o=[b("Default Route",e.default_route_interface??"unknown","BSD interface used by the active default route."),b("Proxy Target",a.length?a.join(", "):"fallback to active services","System Proxy now prefers the default-route service before touching other services.")];return o.push(...t.map(r=>{let n=[r.proxy?.web,r.proxy?.secure_web,r.proxy?.socks].filter(Boolean),l=n.some(m=>m.managed_by_clash),d=!l&&n.some(m=>m.enabled),u=l?"Clash":d?"External":"Off",h=[r.hardware_port,r.bsd_device].filter(Boolean).join(" / ")||r.service_type||"unknown",f=r.is_default_route?"default route":`order ${r.service_order??"-"}`;return b(r.service,`${u} \xB7 ${h}`,`${f}; bypass ${r.bypass_domains?.length??0} domain(s).`)})),F("Network Diagnostics",o)}function It(){return`
    <section class="panel settings-group">
      <div class="section-heading">
        <div>
...
profile:
  ...
  `}function qt(){return`
    <section class="panel settings-group">
      ...
      ${se("Tray Script",s.trayScript,"/usr/bin/say 'Clash for Mac'","Absolute executable path plus arguments; shell expansion is intentionally disabled.","data-tray-script")}
      ${se("Child Process",s.childProcessCommand,"/usr/bin/env","Lifecycle command staged for helper/service mode experiments.","data-child-process-command")}
    </section>
  `}function Ut(){let e=s.payload.product??T.product,t=s.updateInfo,a=t?.available&&t?.version?`New version available: v${c(String(t.version))} (current v${c(String(t.current??e.version??"0.3.3"))}).`:`Current build v${c(e.version??"0.3.3")} \u2014 menu bar Clash for Mac \u2192 Check for Update\u2026 also works.`,o=s.kernelCompare?.comparison,r=o?`<p class="muted">${c(o.narrative?.speed??"")}</p>
    ...
    <div class="feedback-layout">
      <section class="panel hero-panel">
        <div>
          ...
          <h3>${c(e.name)} v${c(e.version??"0.3.3")}${t?.available&&t?.version?` \u2192 v${c(String(t.version))}`:""}</h3>
          <p class="muted">Parity target is CFW 0.20.39; this build is the Apple Silicon beta (${c(e.version??"0.3.3")}).</p>
        </div>
  ...
  `}function Ht(){let e=s.payload.platform??T.platform,t=s.settingsSnapshot??y,a=t.settings??y.settings,o=t.paths??y.paths,r=`${a.external_controller_host}:${a.external_controller_port}`,n=a["interface-name"]??a.interfaceName??"",l=s.networkDiagnostics?.default_route_interface??"auto",d=a.theme==="dark"?"dark":"light",u=a.font_family??a.fontFamily??"",h=[["Security",[b("Core Secret",a.secret?"random UUID":"not set","Protect Clash REST API with a random RFC4122 UUID.")]],["Appearance",[Nt("Theme",d,[{value:"light",label:"Light"},{value:"dark",label:"Dark"}],"Persisted app theme applied at boot.","data-theme-setting"),se("Font",u,"Avenir Next","Override dashboard font family; blank uses the CFW-like default.","data-font-family")]],["System Proxy",[L("systemProxy","System Proxy","Enable macOS system HTTP/HTTPS/SOCKS proxy."),L("proxyDelayIndicator","Tray delay indicator","Show selected-node latency in the menu-bar title/tooltip."),L("usePacScript","Use PAC Script","When System Proxy is on, apply Auto Proxy URL (PAC) instead of manual HTTP/HTTPS/SOCKS."),(()=>{let m=(a.proxy_bypass??[]).join(`
`),P=a.pacScript??a.pac_script??s.pacScript??"";return`
    ...
    <div class="settings-layout">
      ...
      ${F("Core",[b("mixed-port",String(a.mixed_port),"HTTP, HTTPS and SOCKS entry point."),b("external-controller",r,"Local Clash controller endpoint."),b("secret",a.secret?"\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022":"empty","Controller secret stored in cfw-settings.yaml."),`
        ...
        `,L("enableIpv6","IPv6","Expose IPv6 option in generated Clash config."),ue("Install clash-rs","Apple Silicon only","Download pinned clash-rs v0.10.7 beside mihomo (does not change default).","install-pinned-clash-rs","Install")])}
      ${It()}
      ${Bt()}
      ${qt()}
      ...
      ${F("macOS",[b("Minimum macOS",e.minimum_macos??"13.0","ARM64-only app baseline."),b("Intel support",e.intel_supported?"Enabled":"Disabled","Removed to optimize Apple Silicon runtime."),b("System proxy",e.system_proxy_strategy??"native manager","Rust SystemConfiguration boundary."),b("Service Mode",f,"Live SMAppService status for the privileged helper."),ue("Service Mode",f,"Register the SMAppService helper (approve under Login Items if prompted).","manage-service-mode","Manage")])}
      ${Rt()}
      ...
      <section class="panel settings-index">
        <div class="section-heading">
          ...
          <span class="badge">${h.length} groups</span>
        </div>
