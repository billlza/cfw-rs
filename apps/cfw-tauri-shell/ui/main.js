...
`)).split(/[\n,]+/).map(t=>t.trim()).filter(Boolean),tun_mode:s.toggles.tunMode,mixin:s.toggles.mixin,mixin_yaml:document.querySelector("[data-mixin-yaml]")?.value??s.mixinYaml??e.mixin_yaml??"",profile_parser_script:document.querySelector("[data-profile-parser-script]")?.value??s.profileParserScript??e.profile_parser_script??"",tray_script:document.querySelector("[data-tray-script]")?.value?.trim()??s.trayScript??e.tray_script??"",child_process_command:document.querySelector("[data-child-process-command]")?.value?.trim()??s.childProcessCommand??e.child_process_command??"",allow_lan:s.toggles.allowLan,enable_ipv6:s.toggles.enableIpv6,launch_at_login:s.toggles.startAtLogin,silent_start:s.toggles.silentStart,break_connections_on_proxy_change:s.toggles.breakOnProxyChange,show_tray_proxy_delay_indicator:s.toggles.proxyDelayIndicator,usePacScript:s.toggles.usePacScript,pacScript:document.querySelector("[data-pac-script]")?.value??s.pacScript??e.pacScript??e.pac_script??"",theme:document.querySelector("[data-theme-setting]")?.value??e.theme??"light",font_family:document.querySelector("[data-font-family]")?.value?.trim()??e.font_family??"","interface-name":document.querySelector("[data-outbound-interface]")?.value?.trim()??e["interface-name"]??e.interfaceName??"",randomMixedPort:document.querySelector("[data-random-mixed-port]")?.checked??!!(e.randomMixedPort??e.random_mixed_port),delayTestUrl:document.querySelector("[data-delay-test-url]")?.value?.trim()||e.delayTestUrl||e.delay_test_url||"http://www.gstatic.com/generate_204",coreKind:document.querySelector("[data-core-kind]")?.value||e.coreKind||e.core_kind||"clash_rs",core_kind:document.querySelector("[data-core-kind]")?.value||e.coreKind||e.core_kind||"clash_rs"}}function ot(e){let t=e.theme==="dark"?"dark":"light";document.documentElement.dataset.theme=t;let a=String(e.font_family??e.fontFamily??"").trim();document.documentElement.style.setProperty("--sans",a?`"${a}", "Avenir Next", "SF Pro Text", "Helvetica Neue", sans-serif`:'"Avenir Next", "SF Pro Text", "Helvetica Neue", sans-serif')}function rt(e){if(!e)return;let t=e.config??{},a=t.mode?t.mode[0].toUpperCase()+t.mode.slice(1).toLowerCase():null;["Global","Rule","Direct","Script"].includes(a)&&(s.mode=a);let o=t["allow-lan"]??t.allow_lan,r=t["mixed-port"]??t.mixed_port;typeof o=="boolean"&&(s.toggles.allowLan=o),typeof t.ipv6=="boolean"&&(s.toggles.enableIpv6=t.ipv6),(t["log-level"]||t.log_level)&&(s.logLevel=t["log-level"]??t.log_level),typeof r=="number"&&(s.settingsSnapshot.settings.mixed_port=r);let n=new Map((e.proxies?.proxies??[]).map(g=>[g.name,g])),l=e.proxies?.groups??[],d=new Map(l.map(g=>[g.name,g])),u=new Map;s.toggles.testingDelays&&s.proxyGroups.forEach(g=>{g.options.forEach(f=>{u.set(f.name,f.delay)})}),s.proxyGroups=l.map(g=>({name:g.name,type:g.kind,now:g.now??g.options?.[0]??"DIRECT",options:(g.options??[]).map(f=>{let m=n.get(f),k=d.get(f),S=Re(m?.history??k?.history??[]);return s.toggles.testingDelays&&u.has(f)&&(S=u.get(f)),{name:f,delay:S,dead:!1,kind:m?.kind??m?.type??k?.kind??g.kind??"Proxy",udp:m?.udp??null}})})),Fe(e.connections),s.controllerStatus="controller live"}function Fe(e){if(!e)return;let t=Date.now(),a=s.connectionStream.at?Math.max(.25,(t-s.connectionStream.at)/1e3):0,o=e.upload??e.uploadTotal??e.upload_total??0,r=e.download??e.downloadTotal??e.download_total??0,n=s.connectionStream.rows??new Map,l=e.connections??[];s.connections=l.map(d=>{let u=d.metadata??{},g=u.host||u.destinationIP||u.destination_ip||"unknown",f=d.rulePayload??d.rule_payload,m=[d.rule,f].filter(Boolean).join(","),k=n.get(d.id),S=d.upload??0,T=d.download??0,I=k&&a?Math.max(0,(S-k.upload)/a):0,U=k&&a?Math.max(0,(T-k.download)/a):0;return{id:d.id,host:g,rule:m||"MATCH",chains:d.chains??[],upload:x(S),download:x(T),uploadBytes:S,downloadBytes:T,uploadSpeedBytes:I,downloadSpeedBytes:U,speed:`${x(I)}/s up \xB7 ${x(U)}/s down`,age:d.start?d.start.slice(11,19):"live",start:d.start,metadata:u}}),a&&(s.traffic.upload=Math.max(0,(o-s.connectionStream.uploadTotal)/a/1024/1024),s.traffic.download=Math.max(0,(r-s.connectionStream.downloadTotal)/a/1024/1024)),s.connectionStream={at:t,uploadTotal:o,downloadTotal:r,rows:new Map(l.map(d=>[d.id,{upload:d.upload,download:d.download}]))},s.controllerStatus="controller live stream"}function nt(e){if(!e)return!1;let t=e.proxy_providers??[],a=e.rule_providers??[];return s.providers=t.map(o=>({name:o.name,type:o.kind,vehicle:o.vehicle_type,updated:o.updated_at??"unknown",health:o.extra?.healthCheck?.lastResult??o.extra?.healthcheck?.lastResult??"Unknown",proxies:o.proxies?.length??0})),s.ruleProviders=a.map(o=>({name:o.name,type:o.kind,behavior:o.behavior??o.vehicle_type,updated:o.updated_at??"unknown",rules:o.rules?.length??0})),!0}function ke(){let e=pe(s.logSearch);return s.logs.filter(t=>{let a=de(t.level),o=s.logFilter==="all"||a===s.logFilter,r=[t.time,a,t.source,t.message,...(t.fields??[]).map(l=>`${l.key}=${l.value}`)].join(" "),n=!s.logSearch||(e?e.test(r):r.toLowerCase().includes(s.logSearch.toLowerCase()));return o&&n}).slice(0,ie)}function He(){return ke().map(t=>`
        ...
        `).join("")||`<p class="empty">No ${s.logFilter==="all"?"":`${s.logFilter.toUpperCase()} `}logs for this filter.</p>`}function Ce(){let e=document.querySelector(".log-stream");if(!e)return!1;let t=document.querySelector(".logs-layout .toolbar-panel h3");return t&&(t.textContent=`${ke().length} log entries${s.logsPaused?" paused":""}`),e.innerHTML=He(),!0}function Ie(){w.logStreamFrame===null&&(w.logStreamFrame=window.requestAnimationFrame(()=>{w.logStreamFrame=null,s.activePage==="logs"&&(Ce()||se())}))}function ze(){let e=pe(s.connectionSearch),t=s.connections.filter(r=>{let n=r.metadata??{},l=[r.host,r.rule,...r.chains??[],n.processPath,n.process_path,n.sourceIP,n.source_ip,n.destinationIP,n.destination_ip,n.network,n.type].filter(Boolean).join(" ");return!s.connectionSearch||(e?e.test(l):l.toLowerCase().includes(s.connectionSearch.toLowerCase()))}),a={host:r=>r.host,speed:r=>r.uploadSpeedBytes+r.downloadSpeedBytes,upload:r=>r.uploadBytes,download:r=>r.downloadBytes,age:r=>Date.parse(r.start||"")||0},o=a[s.connectionSort]??a.age;return[...t].sort((r,n)=>{let l=o(r),d=o(n),u=typeof l=="string"?l.localeCompare(String(d)):l-d;return s.connectionSortDesc?-u:u}).slice(0,Me)}function it(){let e=pe(s.ruleSearch);return s.rules.filter(t=>{let a=[t.index,t.type,t.payload,t.proxy,t.hits].filter(Boolean).join(" ");return!s.ruleSearch||(e?e.test(a):a.toLowerCase().includes(s.ruleSearch.toLowerCase()))}).slice(0,2e3)}function lt(){let e=document.getElementById("nav");e.innerHTML=s.payload.pages.filter(t=>Pe.has(t.id)).map((t,a)=>`
    ...
    <div class="cfw-row cfw-row-bench">
      <div class="cfw-row-left">
        ...
        <span class="general-icons">
          ${D("show-kernel-compare","info","Show measured clash-rs vs mihomo details")}
        </span>
      </div>
      <div class="cfw-row-right">
  ...
  `}function me(e){!e||typeof e!="object"||(s.updateInfo={available:!!e.available,current:e.current??s.payload?.product?.version,version:e.version??null,notes:e.notes??null,error:e.error??null})}async function Ve(e=!1){if(s.kernelCompare&&!e)return s.kernelCompare;try{return s.kernelCompare=await p("kernel_compare_report"),s.kernelCompare}catch(t){return i("warning","bench",`Kernel compare unavailable: ${t.message??String(t)}`),null}}async function We(e){me(e),E({autoCheck:!0,phase:"idle",result:e})}function E(e={}){let t=s.payload?.product??R.product,a=e.result?{available:!!e.result.available,current:e.result.current??t.version,version:e.result.version??null,notes:e.result.notes??null,error:e.result.error??null,date:e.result.date??null}:s.updateInfo,o=e.phase??(e.checking?"checking":e.installing?"installing":"idle");s.glassDialog={kind:"product-about",payload:{phase:o,checking:o==="checking",installing:o==="downloading"||o==="installing",autoCheck:!!e.autoCheck,progress:e.progress??null,update:a}},_()}function Be(e){return!Number.isFinite(e)||e<0?"\u2014":e<1024?`${Math.round(e)} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(1)} MB`}function Ke(e){let t=e?.phase??(e?.checking?"checking":"idle"),a=e?.update,o=e?.progress;if(t==="checking")return"Checking for updates\u2026";if(t==="downloading"){if(typeof o?.percent=="number"){let r=o.total?` of ${Be(o.total)}`:"";return`Downloading update\u2026 ${o.percent}% (${Be(o.downloaded)}${r})`}return"Downloading update\u2026"}return t==="installing"?"Installing update and restarting\u2026":a?.error?`Update failed: ${a.error}`:a?.available&&a?.version?`Update available: v${a.version}`:a&&a.available===!1?`You\u2019re up to date (v${a.current??s.payload?.product?.version??"\u2014"})`:"Check GitHub releases for new builds."}function Ye(){let e=s.settingsSnapshot?.settings??y.settings,a=(s.payload.product??R.product).version??"0.3.5",o=s.updateInfo,r=o?.available&&o?.version?`<button type="button" class="cfw-update-badge" data-action="check-for-updates" title="Update available \u2014 click to download">\u2192 v${c(String(o.version))}</button>`:"",n=s.coreStatus??B,l=n.state==="Running",d=e.external_controller_host??"127.0.0.1",u=e.external_controller_port??9090,g=e.coreKind??e.core_kind??"clash_rs",f=s.controllerVersion?.version??(l?"Connected":n.state),m=l?"cfw-status-dot on":"cfw-status-dot",k=s.logLevel??e.logLevel??e["log-level"]??"info",S=ee(s.serviceModeStatus),T=s.serviceModeStatus==="Enabled",I=!!(e.randomMixedPort??e.random_mixed_port),U=je(e),J=ct(s.serviceModeStatus);return`
    <div class="cfw-general-view">
      <section class="cfw-header">
        <div class="cfw-app-mark">${Oe()}</div>
        <div class="cfw-title">
      ...
      <section class="cfw-content">
        <div class="cfw-row">
          <div class="cfw-row-left">
            <span>Port</span>
            <span class="general-icons">
              ${D("copy-proxy-exports","terminal","Copy proxy export commands for Terminal")}
              ${D("toggle-random-mixed-port",I?"sync":"sync-off","random mixed port",{active:I})}
            </span>
          </div>
          <div class="cfw-row-right">
            <input class="cfw-number" data-mixed-port type="number" min="1" max="65535" value="${e.mixed_port}" aria-label="mixed-port" ${I?"disabled":""}>
          </div>
        </div>
        ...
        <div class="cfw-row">
          <div class="cfw-row-left">
            <span>Allow LAN</span>
            <span class="general-icons">
              ${D("allow-lan-info","info","Turn on to listen on all interfaces by default, or else only listen on 127.0.0.1. Change Bind Address to pick an interface.")}
              ${D("show-network-interfaces","device-hub","network interfaces")}
            </span>
          </div>
          <div class="cfw-row-right">
          ...
          <div class="cfw-row-right">
            <select class="cfw-select" data-log-level>
              ${["info","warning","error","debug","trace","silent"].map(j=>`
                <option value="${j}" ${k===j?"selected":""}>${j}</option>
              `).join("")}
            </select>
          </div>
        </div>
        ...
        <div class="cfw-row">
          <div class="cfw-row-left">
            ...
            <span class="general-icons">
              ${D("preview-runtime-config","memory","Preview the final configuration file that was submitted to Clash Core")}
              ${D("dns-query","dns","Resolve a host using Clash core")}
              ${D("script-test","play","Test script using by Script mode")}
            </span>
          </div>
          <div class="cfw-row-right">
        ...
        <div class="cfw-row">
          ...
          <div class="cfw-row-right">
            <button class="cfw-text-button" data-action="update-geoip-database" title="${s.geoipStatus?.path?c(s.geoipStatus.path):"Download / refresh GeoIP database"}">${c(Te(s.geoipStatus,s.geoipUpdating))}</button>
          </div>
        </div>
        ...
        <div class="cfw-row">
          <div class="cfw-row-left">
            ...
            <span class="general-icons">
              <span class="general-icon ${J}" title="Service Mode status">${Ge("public")}</span>
            </span>
          </div>
          <div class="cfw-row-right">
            <span class="cfw-link-value">${S}</span>
        ...
        <div class="cfw-row">
          <div class="cfw-row-left">
            <span>TUN Mode</span>
            <span class="general-icons">
              ${D("tun-info","info","To enable this mode, please install Service Mode first!")}
              ${D("tun-settings","settings","Settings")}
              ${D("tun-reset-dns","history","System DNS servers that will be set after TUN Mode is disabled")}
            </span>
          </div>
          <div class="cfw-row-right">
            <span class="cfw-link-value">${De(s.tunRuntime?.tun_mode??s.toggles.tunMode,s.serviceModeStatus,s.tunRuntime)}</span>
            ${K("tunMode","TUN Mode",{disabled:!T})}
          </div>
        </div>
        ...
        <div class="cfw-row">
          <div class="cfw-row-left">
            <span>Mixin</span>
            <span class="general-icons">
              ${D("mixin-info","info","Mixin merges YAML into the generated config before reload. Docs: profile mixin in Settings.")}
              ${D("edit-mixin","settings","Edit Mixin content")}
            </span>
          </div>
  ...
  `}function Xe(e){return e==null?"pending":e<=0?"dead":e<80?"fast":e<180?"mid":"slow"}function Qe(e){return e==null?"Pending":e<=0?"Timeout":`${e} ms`}function qe(){if(typeof document<"u"&&document.hidden)return 2;let e=Number(navigator.hardwareConcurrency)||8;return Math.max(4,Math.min(16,e))}function ve(){w.delayTestGeneration=(w.delayTestGeneration??0)+1,s.toggles.testingDelays=!1}function pt(){let e=document.querySelector("[data-proxy-node-grid]");if(!e)return[];let t=e.getBoundingClientRect(),a=[];return e.querySelectorAll("[data-proxy-node]").forEach(o=>{let r=o.getBoundingClientRect();if(r.bottom>=t.top&&r.top<=t.bottom){let n=o.getAttribute("data-proxy-node");n&&a.push(n)}}),a}function ut(e){let t=new Set(pt()),a=[],o=[];for(let r of e)t.has(r)?a.push(r):o.push(r);return a.length?[...a,...o]:e}function fe(e,t){let a=typeof t=="number"&&Number.isFinite(t)?t:0;s.proxyGroups.forEach(o=>{o.options.forEach(r=>{r.name===e&&(r.delay=a,r.dead=a<=0)})})}function Y(e){let t=e?new Set(e):null;document.querySelectorAll("[data-proxy-delay]").forEach(o=>{let r=o.getAttribute("data-proxy-delay");if(!r||t&&!t.has(r))return;let n=null;for(let l of s.proxyGroups){let d=l.options.find(u=>u.name===r);if(d){n=d.delay;break}}o.className=Xe(n),o.textContent=Qe(n)});let a=document.querySelector('[data-action="delay-test"]');a&&(a.classList.toggle("active",!!s.toggles.testingDelays),a.disabled=!!s.toggles.testingDelays)}function Ue(e){e.forEach(t=>{let a=null;for(let o of s.proxyGroups){let r=o.options.find(n=>n.name===t);if(r){a=r;break}}a&&(a.delay===null||a.delay===void 0)&&fe(t,0)}),Y(e)}function gt(e){return String(e).replace(/[^a-z0-9_-]/gi,"-")}function te(e){return["selector","relay"].includes(String(e??"").toLowerCase())}function ue(e){let t='width="18" height="18" viewBox="0 0 24 24" aria-hidden="true"';switch(e){case"scroll":return`<svg ${t} fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/><circle cx="18.5" cy="18.5" r="3.2" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M20.8 20.8L23 23" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>`;case"report":return`<svg ${t} fill="currentColor"><path d="M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM12 17.3c-.72 0-1.3-.58-1.3-1.3s.58-1.3 1.3-1.3 1.3.58 1.3 1.3-.58 1.3-1.3 1.3zm1-4.3h-2V7h2v6z"/></svg>`;case"report-off":return`<svg ${t} fill="currentColor"><path d="M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM12 17.3c-.72 0-1.3-.58-1.3-1.3s.58-1.3 1.3-1.3 1.3.58 1.3 1.3-.58 1.3-1.3 1.3zm1-4.3h-2V7h2v6z" opacity=".38"/></svg>`;case"delay":return`<svg ${t} fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12a10 10 0 0 1 20 0"/><path d="M5 12a7 7 0 0 1 14 0"/><path d="M8.5 12a3.5 3.5 0 0 1 7 0"/><path d="M12 12V7.5" stroke-width="2"/><circle cx="12" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>`;case"eye":return`<svg ${t} fill="currentColor"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>`;case"eye-off":return`<svg ${t} fill="currentColor"><path d="M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.83l2.92 2.92c1.51-1.26 2.7-2.89 3.43-4.75-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C10.74 7.13 11.35 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.31-.78l3.15 3.15.02-.16c0-1.66-1.34-3-3-3l-.17.01z"/></svg>`;default:return""}}function ft(e){return e?e.dead?!0:typeof e.delay=="number"&&e.delay<=0:!1}function ht(){let e=s.proxyFilter.trim().toLowerCase(),t=s.proxyGroups.map(f=>({...f,options:f.options.filter(m=>{let k=!e||f.name.toLowerCase().includes(e)||m.name.toLowerCase().includes(e),S=s.toggles.hideUnavailable&&ft(m)&&f.now!==m.name;return k&&!S})})).filter(f=>f.options.length||f.name.toLowerCase().includes(e)),a=t.find(f=>f.name===s.activeProxyGroup)??t.find(f=>te(f.type)&&/选择|select|proxy|节点/i.test(f.name))??t.find(f=>te(f.type)&&f.name.toUpperCase()!=="GLOBAL")??t.find(f=>te(f.type))??t[0]??null,o=a?te(a.type):!1,r=!!s.toggles.hideUnavailable,n=s.toggles.showProxiesList!==!1,l=s.proxyBlinkNode,d=s.controllerStatus==="controller live"&&s.proxyGroups.length>0,u=s.controllerStatus==="controller live"&&s.proxyGroups.length===0?"Active profile has no proxy groups. Switch to a subscription with nodes.":"Controller unavailable. No live proxy groups are being displayed.";return`
    <div class="proxy-layout">
      ...
      <div class="mode-switch proxy-mode-header" role="group" aria-label="Proxy mode">
        ${["Global","Rule","Direct","Script"].map(f=>`
          <button class="${s.mode===f?"selected":""}" data-mode="${f}">${f} <span>${mt(f)}</span></button>
        `).join("")}
      </div>`:""}
        ...
        ${a?`
          ...
          <div class="cfw-proxy-content">
            ...
            <div class="cfw-node-grid" data-proxy-node-grid>
              ${a.options.map(f=>`
                <button class="cfw-node-card ${a.now===f.name?"selected":""} ${l===f.name?"blink":""} ${o?"":"readonly"}" data-proxy-node="${c(f.name)}" ${o?`data-group="${c(a.name)}" data-node="${c(f.name)}"`:"disabled"} title="${o?"Select proxy":"This Clash group type is controlled by the core"}">
                  <i></i>
                  <span>
                    <strong>${vt(f.name)}${c(f.name)}</strong>
                    <small>${c(f.kind??"Proxy")} ${f.udp===!1?"":"<em>UDP</em>"}</small>
                  </span>
                  <b class="${Xe(f.delay)}" data-proxy-delay="${c(f.name)}">${Qe(f.delay)}</b>
                </button>
              `).join("")}
            </div>
            ...
            <aside class="cfw-group-rail">
              ${t.map(f=>`
                <button class="${f.name===a.name?"active":""}" data-proxy-group-tab="${c(f.name)}" title="${c(f.name)}">${c(yt(f.name))}</button>
              `).join("")}
            </aside>
          </div>
        `:`<p class="empty">${c(u)}</p>`}
  ...
  `}function mt(e){return{Global:"\u2197",Rule:"\u219D",Direct:"\u2192",Script:"\u21AD"}[e]??""}function vt(e){let t=String(e??"");return/[\u{1F1E6}-\u{1F1FF}]/u.test(t)?"":/^(DIRECT|REJECT)$/i.test(t)?"\u2022 ":""}function yt(e){let t=String(e??"").trim();if(!t)return"?";if(/^GLOBAL$/i.test(t))return"GLOBAL";let a=t.replace(/[\u{1F1E6}-\u{1F1FF}]/gu,"").replace(/[|｜丨&＆/\\()[\]{}【】「」『』·•._\-:：;；,，。!！?？"'“”‘’]+/g,"").replace(/\s+/g,"");return([...a].filter(r=>/[\u4e00-\u9fffA-Za-z0-9]/.test(r)).join("")||a||t).slice(0,6)}function bt(e){try{return new URL(e).host}catch{return e||"local file"}}function wt(e){return e.updateInterval?`interval ${e.updateInterval}`:e.sourceUrl?"remote":"local"}function Je(e){if(!e||typeof e!="string")return null;let t=Object.create(null);for(let g of e.split(/[;\s]+/)){let f=g.indexOf("=");f<=0||(t[g.slice(0,f).trim().toLowerCase()]=g.slice(f+1).trim())}let a=Number(t.upload),o=Number(t.download),r=Number(t.total),n=Number(t.expire),l=(Number.isFinite(a)?a:0)+(Number.isFinite(o)?o:0),d=null;Number.isFinite(r)&&r>0&&(d=Math.max(0,Math.min(100,l/r*100)));let u=null;if(Number.isFinite(n)&&n>0){let g=new Date(n*1e3);if(!Number.isNaN(g.getTime())){let f=m=>String(m).padStart(2,"0");u=`${g.getFullYear()}-${f(g.getMonth()+1)}-${f(g.getDate())}`}}return{used:l,total:Number.isFinite(r)&&r>0?r:null,percent:d,expireDate:u}}function xe(e){if(!Number.isFinite(e)||e<0)return"0B";let t=["B","KB","MB","GB","TB"],a=e,o=0;for(;a>=1024&&o<t.length-1;)a/=1024,o+=1;return o===0?`${Math.round(a)}B`:`${a.toFixed(1)}${t[o]}`}function _t(e){if(!Number.isFinite(e)||e<=0)return"unknown";let t=Date.now()-e*1e3;if(t<0)return new Date(e*1e3).toLocaleString();let a=Math.floor(t/1e3);return a<45?"a few seconds":a<90?"1 minute":a<3600?`${Math.floor(a/60)} minutes`:a<5400?"1 hour":a<86400?`${Math.floor(a/3600)} hours`:new Date(e*1e3).toLocaleString()}function St(e){let t=Je(e.subscriptionUserinfo);if(t&&(t.total!=null||t.expireDate)){let a=[];return t.total!=null?(a.push(`<span>${c(xe(t.used))}</span>`),a.push(`<span>${c(xe(t.total))}</span>`)):a.push(`<span>${c(xe(t.used))}</span>`),t.expireDate&&a.push(`<span>${c(t.expireDate)}</span>`),a.join("")}return`
    <span>${c(e.traffic)}</span>
    <span>${e.rules.toLocaleString()} rules</span>
    <span>${c(wt(e))}</span>
  `}function xt(e){let t=Je(e.subscriptionUserinfo);return t?.percent!=null?t.percent:0}var $t=[{id:"select",label:"Select",icon:"check",needsInactive:!0},{id:"open-web",label:"Open web page",icon:"home",remoteOnly:!1,needsHomeWeb:!0},{id:"edit",label:"Edit",icon:"edit"},{id:"edit-external",label:"Edit externally",icon:"edit"},{id:"update",label:"Update",icon:"refresh",remoteOnly:!0},{id:"reveal",label:"Show in folder",icon:"folder"},{id:"diff",label:"Diff",icon:"diff",remoteOnly:!0},{id:"proxies",label:"Edit proxies section",icon:"send"},{id:"rules",label:"Edit rules section",icon:"rules"},{id:"copy",label:"Copy",icon:"copy"},{id:"qrcode",label:"QRCode",icon:"qr",remoteOnly:!0},{id:"parsers",label:"Parsers",icon:"tree",remoteOnly:!0},{id:"run-script",label:"Run script",icon:"code"},{id:"settings",label:"Settings",icon:"gear"},{id:"delete",label:"Delete",icon:"trash",danger:!0}];function kt(e){let t={check:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M9.2 16.6 4.8 12.2l1.4-1.4 3 3 8.6-8.6 1.4 1.4-10 10z"/></svg>',home:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 3.2 3.8 10.2v9.6h5.4v-5.4h5.6v5.4h5.4v-9.6L12 3.2z"/></svg>',edit:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M4.5 16.9 15.8 5.6l2.6 2.6L7.1 19.5H4.5v-2.6zm14.3-11.7 1.5 1.5c.4.4.4 1 0 1.4l-1.2 1.2-2.6-2.6 1.2-1.2c.4-.4 1-.4 1.4 0z"/></svg>',refresh:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 5a7 7 0 0 1 6.3 4H16v2h5.5V5.5H19v1.7A9 9 0 1 0 21 12h-2a7 7 0 1 1-7-7z"/></svg>',folder:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M3.5 6.5A2 2 0 0 1 5.5 4.5h4l2 2h7a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-13a2 2 0 0 1-2-2v-11z"/></svg>',diff:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M7 4h2v5H7V4zm0 11h2v5H7v-5zm8-7.5 3.5 3.5L15 14.5V12h-4v-2h4V7.5zM5 10h6v2H5v-2z"/></svg>',send:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M3.2 11.2 20 3.5 12.3 20.8l-1.7-6.4-7.4-3.2zm4.4 2.3 4.2 1.8 3.3-7.4-7.5 5.6z"/></svg>',rules:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M5 5h14v2H5V5zm0 6h14v2H5v-2zm0 6h10v2H5v-2z"/></svg>',copy:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M8 7h10v12H8V7zm-3 3H4V4h11v2H5v4zm3-1h2v10h8v2H8V9z"/></svg>',qr:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M4 4h7v7H4V4zm2 2v3h3V6H6zm7-2h7v7h-7V4zm2 2v3h3V6h-3zM4 13h7v7H4v-7zm2 2v3h3v-3H6zm9 0h2v2h-2v-2zm4 0h2v2h-2v-2zm-4 4h2v2h-2v-2zm2 2h4v2h-4v-2zm2-4h2v4h-2v-4z"/></svg>',tree:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M10 3h4v4h-4V3zm-5 7h4v4H5v-4zm10 0h4v4h-4v-4zM7 13h2v3h6v-3h2v5H7v-5z"/></svg>',code:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="m8.2 7.2 1.4 1.4L6.8 12l2.8 3.4-1.4 1.4L4 12l4.2-4.8zm7.6 0L20 12l-4.2 4.8-1.4-1.4 2.8-3.4-2.8-3.4 1.4-1.4z"/></svg>',gear:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M11 3h2l.4 2.2a6.8 6.8 0 0 1 1.8.8l2-1.1 1.4 1.4-1.1 2a6.8 6.8 0 0 1 .8 1.8L20.5 11v2l-2.2.4a6.8 6.8 0 0 1-.8 1.8l1.1 2-1.4 1.4-2-1.1a6.8 6.8 0 0 1-1.8.8L13 20.5h-2l-.4-2.2a6.8 6.8 0 0 1-1.8-.8l-2 1.1-1.4-1.4 1.1-2a6.8 6.8 0 0 1-.8-1.8L3.5 13v-2l2.2-.4a6.8 6.8 0 0 1 .8-1.8l-1.1-2 1.4-1.4 2 1.1a6.8 6.8 0 0 1 1.8-.8L11 3zm1 6.5A2.5 2.5 0 1 0 12 14a2.5 2.5 0 0 0 0-5z"/></svg>',trash:'<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M9 4h6l1 2h4v2H4V6h4l1-2zm1 5h2v9h-2V9zm4 0h2v9h-2V9zM7 9h2v9H7V9z"/></svg>'};return t[e]??t.gear}function A(){s.profileContextMenu=null,s.glassDialog=null,_()}function Ct(e,t,a){s.glassDialog=null,s.profileContextMenu={id:e,x:t,y:a},_()}function _(){let e=document.getElementById("glass-menu-root");if(!e)return;let t=[];if(s.profileContextMenu){let a=s.profiles.find(o=>o.id===s.profileContextMenu.id);if(a){let o=!!a.sourceUrl,n=$t.filter(l=>!(l.needsHomeWeb&&!a.homeWeb||l.remoteOnly&&!o||l.needsInactive&&a.active)).map(l=>`
        <button type="button" class="glass-menu-item ${l.danger?"danger":""}" data-profile-menu="${l.id}" data-profile-id="${c(a.id)}">
          <span class="glass-menu-icon">${kt(l.icon)}</span>
          <span>${c(l.label)}</span>
        </button>
      `).join("");t.push(`
...
[::]:53`)}</textarea></label>
      ...
      `);else if(a.kind==="product-about"){let n=(s.payload?.product??R.product).version??"0.3.5",l=Ke(a.payload),d=a.payload?.update,u=a.payload?.phase??(a.payload?.checking?"checking":"idle"),g=u==="checking"||u==="downloading"||u==="installing",f=!!(d?.available&&d?.version&&!g),m=typeof a.payload?.progress?.percent=="number"?Math.max(0,Math.min(100,a.payload.progress.percent)):null,k=d?.notes?`<p class="product-about-notes">${c(String(d.notes).slice(0,280))}</p>`:"",S=u==="downloading"||u==="installing"?`<div class="product-about-progress" aria-hidden="true"><i style="width:${m??18}%"></i></div>`:"",T=u==="checking"?"Checking\u2026":u==="downloading"?m==null?"Downloading\u2026":`Downloading\u2026 ${m}%`:u==="installing"?"Installing\u2026":f?`Download & Install v${c(String(d.version))}`:"";t.push(`
        <div class="glass-dialog-backdrop" data-glass-dismiss></div>
        <div class="glass-dialog product-about" role="dialog" aria-label="About Clash for Mac">
          <div class="product-about-icon">${Oe()}</div>
...
[::]:53`;try{let l=$(),d=await p("write_settings_snapshot",{settings:{...l,"tun-stack":a,tunStack:a,"tun-auto-route":o,tunAutoRoute:o,"tun-strict-route":r,tunStrictRoute:r,"tun-dns-hijack":n,tunDnsHijack:n}});if(v(d),A(),i("info","tun",`TUN settings saved (stack=${a})`),s.toggles.tunMode)try{await p("reapply_runtime_config"),i("info","tun","Runtime config reapplied (root core respawn when TUN owns core)")}catch(u){i("warning","tun",`Reapply after TUN settings failed: ${u.message??String(u)}`)}}catch(l){i("warning","tun",`TUN settings failed: ${l.message??String(l)}`)}h()})});let e=async t=>{let a=document.querySelector("[data-glass-restore-dns]")?.value??"Empty";try{let o=$(),r=await p("write_settings_snapshot",{settings:{...o,"restore-dns-servers":a,restoreDnsServers:a}});if(v(r),t){let n=await p("apply_restore_dns_servers",{servers:a});i("info","dns",n)}else i("info","dns","Restore DNS list saved");A()}catch(o){i("warning","dns",o.message??String(o))}h()};document.querySelectorAll("[data-glass-restore-dns-save]").forEach(t=>{t.addEventListener("click",()=>e(!1))}),document.querySelectorAll("[data-glass-restore-dns-apply]").forEach(t=>{t.addEventListener("click",()=>e(!0))}),document.querySelectorAll("[data-glass-service-install]").forEach(t=>{t.addEventListener("click",async()=>{try{let a=await p("enable_service_mode");s.serviceModeStatus=a,i("info","service",`Service Mode: ${ee(a)}`)}catch(a){i("warning","service",a.message??String(a))}A(),h()})}),document.querySelectorAll("[data-glass-service-uninstall]").forEach(t=>{t.addEventListener("click",async()=>{try{await p("disable_service_mode"),s.serviceModeStatus=await p("service_mode_status"),i("info","service","Service Mode disabled")}catch(a){i("warning","service",a.message??String(a))}A(),h()})}),document.querySelectorAll("[data-glass-service-login-items]").forEach(t=>{t.addEventListener("click",async()=>{try{await p("open_login_items_settings"),i("info","service","Opened Login Items settings")}catch(a){i("warning","service",a.message??String(a))}})}),document.querySelectorAll("[data-glass-check-update]").forEach(t=>{t.addEventListener("click",async()=>{E({autoCheck:!0,phase:"checking",result:s.updateInfo});try{let a=await p("check_for_updates");me(a),i((a?.available,"info"),"updater",a?.available?`Update ${a.version} available`:`Already up to date (${a?.current??"current"})`),E({autoCheck:!0,phase:"idle",result:a})}catch(a){i("warning","updater",`Update check failed: ${a.message??String(a)}`),E({autoCheck:!0,phase:"idle",result:{available:!1,error:a.message??String(a),current:s.payload?.product?.version}})}})}),document.querySelectorAll("[data-glass-install-update]").forEach(t=>{t.addEventListener("click",async()=>{let a=s.updateInfo?.version??"update";try{i("info","updater",`Downloading and installing ${a}\u2026`),E({autoCheck:!0,phase:"downloading",progress:{downloaded:0,total:null,percent:null},result:{...s.updateInfo,available:!0,error:null}});let o=await p("install_available_update");if(o&&o.installed===!1){let r=o.reason??"no update available";i("warning","updater",`Install skipped: ${r}`),E({autoCheck:!0,phase:"idle",result:{...s.updateInfo,available:!1,error:r}});return}E({autoCheck:!0,phase:"installing",progress:{downloaded:0,total:null,percent:100},result:{...s.updateInfo,available:!0,error:null}})}catch(o){i("warning","updater",`Install failed: ${o.message??String(o)}`),E({autoCheck:!0,phase:"idle",result:{...s.updateInfo,error:o.message??String(o)}})}})})}async function Ze(e,t){let a=s.profiles.find(o=>o.id===t);if(!a)throw new Error(`profile not found: ${t}`);switch(e){case"select":await $e(t);return;case"open-web":{if(!a.homeWeb)throw new Error("no profile-web-page-url for this subscription");await p("open_external_url",{url:a.homeWeb}),i("info","profile",`Opened web page for ${a.name}`);return}case"edit":await q(t,"edit"),i("info","profile",`edit editor opened for ${a.name}`);return;case"proxies":await q(t,"edit","proxies"),i("info","profile",`proxies section editor opened for ${a.name}`);return;case"rules":await q(t,"edit","rules"),i("info","profile",`rules section editor opened for ${a.name}`);return;case"edit-external":await p("open_profile_externally",{id:t}),i("info","profile",`Opened ${a.name} externally`);return;case"update":{if(!a.sourceUrl)throw new Error("local profile has no subscription URL");let o=a.active,r=await p("update_profile",{id:t});await C(),o&&await p("apply_active_profile"),i("info","profile",`${r.name} subscription updated${o?" and config.yaml reapplied":""}`);return}case"reveal":await p("reveal_profile",{id:t}),i("info","profile",`Show in folder: ${a.name}`);return;case"diff":await q(t,"diff"),i("info","profile",`Diff opened for ${a.name}`);return;case"copy":s.glassDialog={kind:"copy",id:t},_();return;case"qrcode":await q(t,"qrcode"),i("info","profile",`QRCode opened for ${a.name}`);return;case"parsers":await q(t,"parsers"),i("info","profile",`Parsers opened for ${a.name}`);return;case"run-script":await q(t,"parsers"),a.active?(await p("apply_active_profile"),i("info","profile",`Run script: reapplied parsers/mixin for active profile ${a.name}`)):i("info","profile",`Run script: parser pipeline runs on apply; select ${a.name} to execute against config.yaml`);return;case"settings":s.glassDialog={kind:"settings",id:t},_();return;case"delete":s.glassDialog={kind:"delete",id:t},_();return;default:throw new Error(`unknown profile menu action: ${e}`)}}function Lt(){return`
        ...
        ${s.profiles.length?s.profiles.map(e=>`
          <article class="cfw-profile-card ${e.active?"active":""}" data-profile-card="${c(e.id)}">
            ...
            <div class="profile-card-main">
              <h3>${c(e.name)}</h3>
              <p>${c(e.sourceUrl?bt(e.sourceUrl):"local file")} (${c(e.updated)})</p>
              <div class="profile-usage">
                ${St(e)}
              </div>
              <div class="profile-progress" title="${e.subscriptionUserinfo?c(e.subscriptionUserinfo):"No subscription usage quota"}"><b style="width:${xt(e).toFixed(1)}%"></b></div>
            </div>
  ...
  `}function At(){let e=s.profileInspector;if(!e)return"";let t=e.profile??{},a=`${t.name??e.id} \xB7 ${e.mode}`,o=t.source_url??t.sourceUrl??null,r=t.body?[...t.body.matchAll(/^([A-Za-z0-9_-]+):/gm)].map(l=>l[1]).filter(l=>l.toLowerCase().includes("parser")):[],n="";return e.mode==="diff"?n=`
  ...
  `}function Et(){let e=s.providerBulkActions.has("update-all-providers"),t=s.providerBulkActions.has("health-check-all");return`
  ...
  `}function Tt(e){let t=e.metadata?.processPath??e.metadata?.process_path??e.processPath??"";if(!t)return"\u2014";let a=String(t).split(/[/\\]/).filter(Boolean);return a[a.length-1]||String(t)}function et(e,t){return`
  ...
  `}function Rt(){let e=ze(),t=s.connections.find(n=>n.id===s.connectionDetailId),a=x(s.connectionStream.uploadTotal),o=x(s.connectionStream.downloadTotal),r=s.toggles.showProcess!==!1;return w.connectionRowEls=null,`
  ...
  `}function It(){let e=document.querySelector("[data-connections-root]"),t=document.querySelector("[data-conn-scroll]");if(!e||!t||s.activePage!=="connections"){h();return}let a=ze(),o=s.toggles.showProcess!==!1,r=e.querySelector("[data-conn-totals]");r&&(r.textContent=`Total: \u2191 ${x(s.connectionStream.uploadTotal)} \u2193 ${x(s.connectionStream.downloadTotal)}`);let n=e.querySelector("[data-conn-close-all]");n&&(n.disabled=!!s.closingAllConnections,n.textContent=s.closingAllConnections?"Closing...":`Close All (${a.length})`),w.connectionRowEls instanceof Map||(w.connectionRowEls=new Map,t.querySelectorAll("[data-connection-id]").forEach(m=>{w.connectionRowEls.set(m.getAttribute("data-connection-id"),m)}));let l=new Set(a.map(m=>m.id));for(let[m,k]of[...w.connectionRowEls.entries()])l.has(m)||(k.remove(),w.connectionRowEls.delete(m));let d=document.createDocumentFragment(),u=!1;a.forEach((m,k)=>{let S=w.connectionRowEls.get(m.id);if(!S){let Z=document.createElement("div");Z.innerHTML=et(m,o).trim(),S=Z.firstElementChild,w.connectionRowEls.set(m.id,S),u=!0,d.appendChild(S);return}let T=S.querySelector("[data-conn-up]"),I=S.querySelector("[data-conn-down]"),U=S.querySelector("[data-conn-speed]");T&&(T.textContent=`\u2191 ${m.upload}`),I&&(I.textContent=`\u2193 ${m.download}`),U&&(U.textContent=m.speed??"0 B/s");let J=S.querySelector("[data-close-connection]");if(J){let Z=s.closingConnectionIds.has(m.id);J.disabled=Z,J.textContent=Z?"Closing":"Close"}let j=t.children[k];!u&&j!==S&&t.insertBefore(S,j??null)}),d.childNodes.length&&t.appendChild(d);let g=s.connections.find(m=>m.id===s.connectionDetailId),f=e.querySelector(".modal-backdrop");g&&!f?(e.insertAdjacentHTML("beforeend",tt(g)),at()):!g&&f&&f.remove()}function Bt(){w.connectionsPatchFrame===null&&(w.connectionsPatchFrame=window.requestAnimationFrame(()=>{w.connectionsPatchFrame=null,It()}))}function tt(e){let t=e.metadata??{};return`
  ...
  `}function Ut(){let e=s.networkDiagnostics;if(!e)return F("Network Diagnostics",[b("Default Route","Unavailable","Run inside the Tauri app to inspect macOS service order.")]);let t=e.services??[],a=e.recommended_clash_proxy_services??[],o=[b("Default Route",e.default_route_interface??"unknown","BSD interface used by the active default route."),b("Proxy Target",a.length?a.join(", "):"fallback to active services","System Proxy now prefers the default-route service before touching other services.")];return o.push(...t.map(r=>{let n=[r.proxy?.web,r.proxy?.secure_web,r.proxy?.socks].filter(Boolean),l=n.some(m=>m.managed_by_clash),d=!l&&n.some(m=>m.enabled),u=l?"Clash":d?"External":"Off",g=[r.hardware_port,r.bsd_device].filter(Boolean).join(" / ")||r.service_type||"unknown",f=r.is_default_route?"default route":`order ${r.service_order??"-"}`;return b(r.service,`${u} \xB7 ${g}`,`${f}; bypass ${r.bypass_domains?.length??0} domain(s).`)})),F("Network Diagnostics",o)}function Nt(){return`
    <section class="panel settings-group">
      <div class="section-heading">
        <div>
...
profile:
  ...
  `}function zt(){let e=s.payload.product??R.product,t=s.updateInfo,a=t?.available&&t?.version?`New version available: v${c(String(t.version))} (current v${c(String(t.current??e.version??"0.3.5"))}).`:`Current build v${c(e.version??"0.3.5")} \u2014 menu bar Clash for Mac \u2192 Check for Update\u2026 also works.`,o=s.kernelCompare?.comparison,r=o?`<p class="muted">${c(o.narrative?.speed??"")}</p>
    ...
    <div class="feedback-layout">
      <section class="panel hero-panel">
        <div>
          ...
          <h3>${c(e.name)} v${c(e.version??"0.3.5")}${t?.available&&t?.version?` \u2192 v${c(String(t.version))}`:""}</h3>
          <p class="muted">Parity target is CFW 0.20.39; this build is the Apple Silicon beta (${c(e.version??"0.3.5")}).</p>
        </div>
  ...
  `}function Gt(){let e=s.payload.quality??R.quality,t=e.performance,a=e.stability;return`
