...
export function preferencesFromAutomationDraft(draft) {
  ...
  if (draft.enabled && !draft.rules.length) throw new Error("Add a network rule before enabling automatic changes");
  const network_rules = draft.rules.map((rule) => {
    if (!MODES.some(([mode]) => mode === rule.mode)) throw new Error("Choose a supported network mode");
    if (!["wifi", "wired", "ssid"].includes(rule.network.type)) throw new Error("Choose a supported network match");
    if (rule.network.type === "ssid" && (!rule.network.name || new TextEncoder().encode(rule.network.name).length > 32)) {
      throw new Error("Enter a Wi-Fi name containing 1–32 UTF-8 bytes");
    }
...
export function createAutomationSettingsUI({ state, invoke, renderPage, appendLog, dismissOtherDialogs }) {
  ...
  async function save() {
    ...
    try {
      ...
      appendLog("info", "settings", "Global shortcuts and network rules saved");
    } catch (error) {
      dialog.error = errorText(error);
      appendLog("error", "settings", `Automation settings were not applied: ${dialog.error}`);
  ...
  function renderDialog() {
    ...
    const networkLabel = snapshot.network_error ?? (network ? `${network.kind === "wifi" ? "Wi-Fi" : "Wired"} · ${network.interface}${network.ssid ? ` · ${network.ssid}` : ""}` : "No active physical network");
    return `<div class="glass-dialog-backdrop" data-automation-dismiss></div>
      <section class="glass-dialog automation-settings-dialog" role="dialog" aria-modal="true" aria-labelledby="automation-title">
        <h2 id="automation-title">Shortcuts & network rules</h2>
        <p class="glass-dialog-copy">Global shortcuts work while this app is in the background. Include Command, Option or Control; leave a field empty to disable it.</p>
        ${SHORTCUT_ACTIONS.map(([action, label]) => `<label class="glass-input-label">${label}<input class="glass-input" data-shortcut-action="${action}" value="${escapeHtml(draft.shortcuts[action] ?? "")}" placeholder="Command+Option+P"${disabled}></label>`).join("")}
        <label class="glass-input-label"><input type="checkbox" data-network-enabled${draft.enabled ? " checked" : ""}${disabled}> Apply network rules automatically</label>
        <p class="glass-dialog-copy">The first matching rule applies after a network change or when enabled. Manual Stop stays stopped on the same network. Rules run while CFM is open.</p>
        <p class="glass-dialog-copy">Current network: ${escapeHtml(networkLabel)}</p>
        ${network?.kind === "wifi" && !network.ssid ? `<p class="glass-dialog-copy">macOS is not providing the Wi-Fi name. Rules for a named Wi-Fi network cannot match until the name is available.</p><button type="button" class="glass-btn ghost" data-wifi-permission${disabled}>Allow Wi-Fi name access…</button>` : ""}
        <button type="button" class="glass-btn ghost" data-network-refresh${disabled}>Refresh network status</button>
        ...
        <div class="automation-rule-list">${draft.rules.map((rule, index) => `<div class="automation-rule-row">
          <select aria-label="Network match ${index + 1}" data-network-match="${index}"${disabled}>${[["wifi", "Any Wi-Fi"], ["ssid", "Named Wi-Fi"], ["wired", "Wired"]].map(([value, label]) => `<option value="${value}"${rule.network.type === value ? " selected" : ""}>${label}</option>`).join("")}</select>
          ${rule.network.type === "ssid" ? `<input class="glass-input" aria-label="Wi-Fi name ${index + 1}" data-network-name="${index}" value="${escapeHtml(rule.network.name ?? "")}" placeholder="Exact Wi-Fi name"${disabled}>` : ""}
          <select aria-label="Network action ${index + 1}" data-network-mode="${index}"${disabled}>${MODES.map(([value, label]) => `<option value="${value}"${rule.mode === value ? " selected" : ""}>${label}</option>`).join("")}</select>
          <button type="button" data-network-up="${index}" aria-label="Move rule ${index + 1} up"${saving || index === 0 ? " disabled" : ""}>↑</button>
          <button type="button" data-network-remove="${index}" aria-label="Remove rule ${index + 1}"${disabled}>Remove</button>
        </div>`).join("")}</div>
        <button type="button" class="glass-btn ghost" data-network-add${saving || draft.rules.length >= 16 ? " disabled" : ""}>Add network rule</button>
        ${dialog.error ?? snapshot.error ? `<p class="glass-dialog-copy warning" role="alert">${escapeHtml(dialog.error ?? snapshot.error)}</p>` : ""}
        <div class="glass-dialog-actions"><button class="glass-btn ghost" data-automation-dismiss${disabled}>Cancel</button><button class="glass-btn primary" data-automation-save${disabled}>${saving ? "Applying…" : "Apply"}</button></div>
      </section>`;
  }
  function bindDialog() {
    const dialog = state.automationDialog;
    ...
    bind("[data-wifi-permission]", "click", async () => {
      try { await invoke("request_wifi_name_access"); dialog.notice = "Respond to the macOS permission prompt, then refresh network status."; }
