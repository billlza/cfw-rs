    ...
    <div class="rules-layout">
      <section class="panel toolbar-panel">
        ...
        <div class="toolbar-actions">
          <button class="button ghost" data-action="reload-rules">${escapeHtml(t("Reload Rules"))}</button>
        </div>
      </section>

      ${pages > 1 ? `<nav class="proxy-pagination" aria-label="${escapeHtml(t("Rule pages"))}"><button data-rule-page="-1"${page === 0 ? " disabled" : ""}>${escapeHtml(t("Previous"))}</button><span>${page + 1} / ${pages}</span><button data-rule-page="1"${page + 1 === pages ? " disabled" : ""}>${escapeHtml(t("Next"))}</button></nav>` : ""}
      <section class="panel table-panel">
        <div class="connection-table">
          <div class="table-row rule-head">
            <span>#</span><span>${escapeHtml(t("Type"))}</span><span>${escapeHtml(t("Payload"))}</span><span>${escapeHtml(t("Proxy"))}</span><span>${escapeHtml(t("Hits"))}</span>
          </div>
          ${rules.map((rule) => `
