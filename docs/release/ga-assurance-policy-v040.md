...
Build 40033 is the only application identity signed, notarized, installed,
accepted, and published for the v0.4.0 GA. The DMG and updater archive will be
two distribution envelopes around the exact same 40033 application tree, not
separate application candidates.

Build 40030 must not be built, signed, submitted to Apple, installed, or used as
a substitute for 40033 evidence. It is recorded as
`retired_unbuilt_policy_superseded`; 40033 is the single `active_ga` build.
...
must not be rebuilt, resumed, promoted, installed, or substituted for 40033.
...
resumed, rebuilt, promoted, installed, or substituted for 40033.
...
- an exact-DMG install and 40019-to-40033 migration on a fixed Apple Silicon
...
1. `prepackage` verifies the exact 40033 application, source and CI identity,
...
1. replaces the validation/final pair with one enforced `ga_build=40033`;
2. keeps build 40030 unbuilt and builds 40031/40032 consumed, failed, and
   permanently outside every active notarization, installation, and review path;
3. performs the existing guarded 40019-to-40033 migration directly;
...
immutable 40033 prepackage, GA-acceptance, and publication stages have each
