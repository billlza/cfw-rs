...
当前工作区包含开始前已有的 65 个发布相关修改；本轮保留了这些内容，没有将它们
擅自提交、覆盖或并入新安装包，也没有绕过签名、公证和安装验收。
40071 未构建，40072 未分配；本轮新源码没有在当前运行网络上热替换。

故障电脑可以独立运行 `scripts/collect_cfm_diagnostics.command`，无须 Rust、Node 或源码仓库。
操作与材料范围见 [无响应现场采集说明](diagnostics-macos.md)。得到调用栈后，按最后一个
未完成的启动步骤和阻塞栈定位具体责任层，再进行针对性修复与同机型复测。
