...
## 本轮最新进度（2026-09-20，40072 已安装）

40071 已签名、公证并安装，启动隔离和状态恢复修正已在安装环境验证。
但其 15 分钟网络采样有 **18/508 次失败**，因此不能作为断流修复完成的交付。
绑定物理 en0 直连同一 SOCKS 节点的并行测量发现，正常协商会耗时 6.5 或 13 秒，
而 CFM 的 SOCKS 封装在五秒提前取消。另一个真实 DNS/SOCKS 测试证明，该预算还会让
备用 DNS 在被调用前就失去执行机会。

40072 将默认 SOCKS 建立过程改用已有的协议握手预算，底层 TCP socket 的建连上限和
显式超时/取消/认证失败处理保留。旧实现失败、新实现通过的真实网络回归已完成，
完整 libbox 测试、race、vet、355 项升级策略测试、605 项签名/源码策略测试以及
11 项原生产品输入测试均通过。

40072 已完成签名、公证、安装及服务重新启用；40071 → 40072 的安装/服务日志导出验证通过。
09:43 恢复工作时，本机已经在运行 40072，CFW 没有运行。本轮直接在现有连接上验收，没有再次切换。
完整应用摘要为 `5951216697f671fc241605cb97b895e5ad8a668ca6601c92ab2efdaa5173beb1`，
09:46 重新通过 codesign / Gatekeeper 检查，活动 Packet Tunnel 扩展为 40072。
TUN 和系统代理（127.0.0.1:7891）开启，IPv6 DNS 保持关闭。

纯 TUN 和系统代理各完成 Google、Apple、X 以及精确 1 MiB 下载，共 8/8 成功。
设置页和原生菜单可用，重载 dashboard 后仍为 Connected。
09:44:14–09:59:12 的 15 分钟采样完成：纯 TUN 与系统代理各 286/286 成功，合计 **572 次、0 失败**。
两条路径还各完成约 64 秒、精确 8 MiB 持续下载。额外 16 MiB 请求曾返回 HTTP 403，
绕过 CFM 直连同节点仍返回相同状态；失败样本及目标限制对照保留，未将其计为成功。
本轮测试进程及临时防休眠进程已停止，40072 继续保持联网。
安装后的成功采样不能替代旧实现失败、新实现通过的因果验证，也不覆盖隔夜未采样时段。

详细记录：`target/startup-architecture-20260919/修复验收记录.md`。
40071 的所有失败样本、旧源码、签名应用及升级日志保留，40072 不复用它的候选身份。
其他故障机的黑屏现场、睡眠唤醒、Wi-Fi 切换和长时间流量仍未完成验收。
...
**以下为初轮历史记录。** 当时安装为 **0.4.0 / 40070**。用户授权了网络切换和安装候选版本；
