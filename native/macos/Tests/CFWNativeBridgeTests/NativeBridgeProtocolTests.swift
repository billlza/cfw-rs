...
@Test func networkAuthorizationCommandsReachTheNativeDispatcherWithoutPayload() throws {
  for command in [NativeBridgeCommand.authorizeSystemProxy, .authorizeSystemProxyRestoration] {
    let original = NativeRequestEnvelope(command: command)
    let decoded = try NativeBridgeProtocolCodec.decodeRequest(JSONEncoder().encode(original))
