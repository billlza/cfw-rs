...
# The GA migration uses the exact retained predecessor admitted by the install
# producer. Runtime acceptance imports this same identity instead of carrying a
# second, independently updated build literal.
PREVIOUS_BUILD: Final = install.INSTALLED_40072_PREDECESSOR.build_number
...
def self_check() -> None:
    if (
        (PRODUCT_VERSION, PREVIOUS_BUILD, GA_BUILD) != ("0.4.0", "40072", "40073")
        or MIGRATION_RELATIVE
        != Path(
            "target/candidates/0.4.0/ga/40073/stage-inputs/"
