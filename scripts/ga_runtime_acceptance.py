...
40048 -> 40060 journals. ``collect`` creates a durable CSPRNG challenge intent,
...
def _require_fixed_paths(
    ...
    if Path(acceptance_path).absolute() != expected_acceptance:
        raise _error("GA runtime adapter path is not the fixed 40060 path")
    if Path(raw_evidence_root).absolute() != expected_raw:
        raise _error("GA runtime raw-evidence path is not the fixed 40060 path")
...
def _validate_expected(value: object) -> dict[str, Any]:
    ...
    if (
        ...
        raise _error("GA runtime expected identity or check set differs from 0.4.0/40060")
    for field in (
        "dmg_gatekeeper_sha256",
        "dmg_set_seal_sha256",
        "dmg_sha256",
...
def _installed_candidate_tree(repository: Path, expected: dict[str, Any]) -> str:
    ...
    if (
        ...
        raise _error("GA install journal is not the completed 40048 to 40060 install")
    try:
        installed = dormant_app_install.read_app_identity(INSTALLED_APP)
    except dormant_app_install.InstallError as error:
        raise _error("installed 40060 application tree cannot be identified") from error
...
def _validate_exact_dmg_install(
    ...
    if (
        ...
        raise _error("DMG-contained app and installed 40060 app are not the same tree")
...
def _running_host_observation(value: object) -> list[dict[str, Any]]:
    ...
    if len(app_processes) != 1:
        raise _error("raw process table does not contain exactly one installed 40060 Host")
    return processes
...
def _host_absence_observation(value: object) -> list[dict[str, Any]]:
    ...
    if any(
        ...
        raise _error("normal shutdown evidence still contains the installed 40060 Host")
    return processes
...
def _validate_launch(value: dict[str, Any]) -> None:
    ...
    launch = _command(
        ...
        expected_exit=0,
        label="installed 40060 launch command",
    )
    if launch["stderr"]:
        raise _error("installed 40060 launch command emitted an error")
...
def _require_launchctl_running(
    ...
    *,
    ...
    inside the installed 40060 bundle, registered through ServiceManagement,
    ...
    try:
        ...
        raise _error(
            f"fixed {label} executable is outside the installed 40060 bundle"
        ) from error
...
def _validate_system_extension(value: dict[str, Any]) -> None:
    ...
    if (TEAM_ID, PACKET_EXTENSION_BUNDLE_ID) not in identities:
        raise _error("raw system extension output lacks the fixed 40060 extension")
    matching = [
        line
            ...
            try:
                ...
                if waiting_for_operator and not announced_operator_boundary:
                    print(
                        ...
                        "Tunnel mode in the installed 40060 dashboard",
                        file=sys.stderr,
                        flush=True,
                    )
...
def self_check() -> None:
    ...
    if (
        (PRODUCT_VERSION, FROM_BUILD, TO_BUILD) != ("0.4.0", "40048", "40060")
        ...
        != Path(
            "target/candidates/0.4.0/ga/40060/stage-inputs/ga-acceptance/"
