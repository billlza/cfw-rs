...
libbox_socks_lifecycle_patch_path() {
  libbox_repository_relative_path "$1" "$SING_BOX_SOCKS_LIFECYCLE_PATCH_PATH" "sing-box SOCKS lifecycle patch"
}

libbox_validate_patches() {
  local repo_root="$1"
  local security_patch_path raw_packet_patch_path dns_failover_patch_path endpoint_conflict_patch_path profile_probe_patch_path socks_lifecycle_patch_path
  ...
  socks_lifecycle_patch_path="$(libbox_socks_lifecycle_patch_path "$repo_root")" || return 1
  ...
  libbox_require_regular_file "$socks_lifecycle_patch_path" || return 1
  if [[ "$(libbox_sha256 "$socks_lifecycle_patch_path")" != "$SING_BOX_SOCKS_LIFECYCLE_PATCH_SHA256" ]]; then
    echo "error: sing-box SOCKS lifecycle patch digest mismatch" >&2
    return 1
  fi
...
libbox_validate_upstream_source() {
  ...
  if ! libbox_git "$source_root" apply --whitespace=error-all --unidiff-zero --check \
    ...
    "$socks_lifecycle_patch_path"; then
...
libbox_validate_patched_source() {
  local repo_root="$1"
  local source_root="$2"
  local actual_diff actual_combined_diff ignored_files security_patch_path raw_packet_patch_path dns_failover_patch_path endpoint_conflict_patch_path profile_probe_patch_path socks_lifecycle_patch_path
  libbox_validate_patches "$repo_root" || return 1
  libbox_validate_git_root "$source_root" || return 1
