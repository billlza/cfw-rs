...
exact 40049 identity.  State mutation is owned by ``orchestrator``; this module
...
def _verify_publication_adapter(repository: Path) -> None:
    ...
    if (
        ...
        raise PublicationError(
            "GA publication verifier adapter is not migrated to ga/40049; "
...
def _verified_prepackage_inputs(
    ...
    *,
    ...
    if (PRODUCT_VERSION, GA_BUILD) != ("0.4.0", "40049"):
        raise PublicationError("prepackage requires the fixed v0.4.0/40049 identity")
    ...
    if (
        ...
        raise PublicationError("signed application is not exactly v0.4.0/40049")
...
def _require_artifact_set_adapter(repository: Path):
    ...
    if release_artifact_set.CANDIDATE_APP_RELATIVE != expected:
        raise PublicationError(
            "GA package verifier adapter is not migrated to ga/40049; "
...
def _require_migration_matches_prepackage(
    ...
    if not closed_migration:
        raise PublicationError(
            "install journal is not a closed 40048 to 40049 migration"
        )
...
def self_check(repository: Path) -> None:
    ...
    if (
        ...
        or ACTIVE_RELEASE_IDENTITY.ga_build != "40049"
        or _path(repository, GA_ROOT)
        != repository / "target/candidates/0.4.0/ga/40049"
