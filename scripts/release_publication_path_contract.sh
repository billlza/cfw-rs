...
cfw_require_fixed_publication_app_path() {
  ...
  expected_app="$repository/target/candidates/0.4.0/ga/40035/signed/Clash for Mac.app"
  if [[ "$canonical_app" != "$expected_app" ]]; then
    echo "error: publication gate accepts only the fixed 0.4.0/40035 GA app: $expected_app" >&2
    return 1
  fi
}
