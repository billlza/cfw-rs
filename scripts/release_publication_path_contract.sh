...
cfw_require_fixed_publication_app_path() {
  ...
  publication_contract_expected_app="$publication_contract_repository/target/candidates/0.4.0/ga/40047/signed/Clash for Mac.app"
  if [[ "$publication_contract_canonical_app" != "$publication_contract_expected_app" ]]; then
    echo "error: publication gate accepts only the fixed 0.4.0/40047 GA app: $publication_contract_expected_app" >&2
    return 1
  fi
}
