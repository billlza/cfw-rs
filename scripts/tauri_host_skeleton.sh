...
if sys.argv[3] == "--native-ui-preview":
    if base_config.get("version") != "0.5.0" or sys.argv[4] != "50010":
        raise SystemExit("error: native UI Host requires exact preview version/build 0.5.0/50010")
    if override["bundle"]["macOS"].get("bundleVersion") != "50010":
        raise SystemExit("error: native UI Host override must retain preview build 50010")
PY
