...
ga_app="$fixture_root/target/candidates/0.4.0/ga/40034/signed/Clash for Mac.app"
mkdir -p "$ga_app"
...
grep -Fq "only the fixed 0.4.0/40034 GA app" "$fixture_root/old-path.stderr"

for retired_build_number in 40030 40031 40032 40033; do
  ...
  grep -Fq "only the fixed 0.4.0/40034 GA app" "$fixture_root/retired-build-$retired_build_number.stderr"
done
