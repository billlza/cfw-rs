...
ga_app="$fixture_root/target/candidates/0.4.0/ga/40063/signed/Clash for Mac.app"
mkdir -p "$ga_app"
...
gate_artifact="$gate_fixture/target/release-worktrees/40063"
gate_ga_root="$gate_artifact/target/candidates/0.4.0/ga/40063"
...
cfw_run_release_python_script() {
  case "$2" in
    "$1/scripts/release_executor_source.py")
      ...
      printf '%s\n' "$1/target/release-worktrees/40063"
      ;;
    ...
    "$1/scripts/github_hosted_ci_receipt.py")
      [[ "$1" != */target/release-worktrees/40063 ]] || return 1
      if [[ "$3" == "capture" ]]; then
...
fixture_ga_root="$fixture_artifact_root/target/candidates/0.4.0/ga/40063"
[[ "$#" -eq 4 ]]
...
grep -Fq "only the fixed 0.4.0/40063 GA app" "$fixture_root/old-path.stderr"
...
for retired_build_number in 40030 40031 40032 40033 40034 40035 40036 40037 40038 40039 40040 40041 40042 40043 40044 40045; do
  ...
  grep -Fq "only the fixed 0.4.0/40063 GA app" "$fixture_root/retired-build-$retired_build_number.stderr"
done
