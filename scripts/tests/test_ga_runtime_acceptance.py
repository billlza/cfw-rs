...
def launchctl_output(
    *,
    ...
    service_label: str,
    parent_bundle_version: str = "40048",
) -> str:
...
def system_extension_output() -> str:
    return (
        ...
        "(0.4.0/40048)\tCFWPacketTunnel\t[activated enabled]\n"
    )
...
class GARuntimeAcceptanceTests(unittest.TestCase):
    ...
    def test_contract_has_fixed_paths_and_twelve_raw_derived_checks(self) -> None:
        self_check()
        self.assertEqual((PRODUCT_VERSION, FROM_BUILD, TO_BUILD), ("0.4.0", "40047", "40048"))
        self.assertEqual(
    ...
    def test_ordinary_command_keeps_the_fifteen_minute_duration_bound(self) -> None:
        ...
        with self.assertRaisesRegex(
            GARuntimeAcceptanceError,
            "installed 40048 launch command command identity, exit, or duration is invalid",
        ):
            self.fixture.seal()
    ...
    def test_service_registration_rejects_a_different_parent_bundle_build(self) -> None:
        ...
        for build in ("40041", "40043", "40044", "40045", "40046", "40047"):
            with self.subTest(build=build):
                self._set_service_stdout(
                    "proxy_agent",
                    original.replace(
                        "parent bundle version = 40048",
