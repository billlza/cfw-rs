...
class GAStageExecutorSourceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.expected_freeze_verifier: FreezeVerifier | None = None
    ...
    def _prepackage_files(
        ...
        *,
        ...
        self.assertIs(freeze_verifier, self.expected_freeze_verifier)
    ...
    def test_each_stage_keeps_its_own_sealer_across_later_policy_commits(self) -> None:
        freeze_verifier = Mock(
            side_effect=AssertionError("source provenance fixture does not execute artifact validation")
        )
        self.expected_freeze_verifier = freeze_verifier
        prepackage = seal_prepackage(
            self.repository, executor=self.sealer, freeze_verifier=freeze_verifier
        )
        prepackage_bytes = self.manifest_path.read_bytes()
        with (
            patch.object(
                contract, "_ga_acceptance_files", side_effect=self.fixture.ga_acceptance_files
            ) as acceptance_composition,
            patch.object(
                contract, "_publication_files", side_effect=self.fixture.publication_files
            ) as publication_composition,
        ):
            self._commit_policy("acceptance policy")
            acceptance_executor = capture_executor_source(self.repository)
            acceptance = seal_ga_acceptance(
                self.repository,
                executor=acceptance_executor,
            ...
            publication = seal_publication(
                self.repository,
                executor=publication_executor,
                freeze_verifier=freeze_verifier,
            )
            self._commit_policy("later read-only verifier policy")
            self.assertEqual(
                contract.verify_stage(
                    self.repository, "publication", freeze_verifier=freeze_verifier
                ),
                publication,
            )

        for composition in (self.compose, acceptance_composition, publication_composition):
            self.assertGreater(composition.call_count, 0)
            for call in composition.call_args_list:
                self.assertIs(call.kwargs["freeze_verifier"], freeze_verifier)
        freeze_verifier.assert_not_called()
    ...
    def test_verifier_drift_during_recomposition_is_rejected(self) -> None:
        ...
        def drift(
            ...
            *,
            ...
            files = self._prepackage_files(
                repository,
                executor_source,
                expected_live_hosted_ci=expected_live_hosted_ci,
                freeze_verifier=freeze_verifier,
            )
