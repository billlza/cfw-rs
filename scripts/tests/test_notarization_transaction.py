...
class Fixture:
    def __init__(self) -> None:
        ...
        self.build = self.candidate / "ga/40050"
        ...
        self.signing_transformation_receipt = {
            ...
            "product": {"build_number": "40050", "version": "0.4.0"},
            "profiles": {
                "host": "c" * 64,
                "packet_tunnel": "d" * 64,
                "proxy_agent": "e" * 64,
...
class NotarizationTransactionSuccessTests(unittest.TestCase):
    ...
    def test_ga_uses_the_fixed_candidate_destinations(self) -> None:
        ...
        self.assertEqual(
            final_app,
            self.fixture.candidate / "ga/40050/signed/Clash for Mac.app",
        )
        self.assertEqual(
            context.native_products,
            (
                self.fixture.candidate
                / "ga/40050/signing-output/signed-native-products"
            ),
        )
        self.assertEqual(
            context.attempt_root,
            self.fixture.candidate / "ga/40050/transactions/app-notary",
        )
        manifest = json.loads(
        ...
        self.assertEqual(manifest["metadata"]["buildNumber"], "40050")
...
class NotarizationRecoveryTests(unittest.TestCase):
    ...
    def test_current_ga_fixture_continues_once_then_recovers_locally(self) -> None:
        ...
        self.assertEqual(fixture.context.build_number, "40050")
        self.assertEqual(
            fixture.context.archive_name,
            "Clash.for.Mac_0.4.0_40050_notary.zip",
        )
        ...
        self.assertEqual(
            first,
            fixture.candidate / "ga/40050/signed/Clash for Mac.app",
        )
        ...
        self.assertEqual(final_manifest["metadata"]["buildNumber"], "40050")
        self.assertEqual(
...
repo_root=/release
candidate_root=/release/target/candidates/0.4.0/ga/40050
...
"""
    ...
    def _notary_call(*mode: str) -> list[str]:
        return [
            ...
            *mode,
            "--native-products",
            "/release/target/candidates/0.4.0/ga/40050/signing-output/signed-native-products",
            "--notary-profile",
            "clashformac-notary",
            "--repository-commit",
            "a" * 40,
    ...
    def test_build_and_signing_resume_keep_full_signing_verification(self) -> None:
        staged = (
            "--staged-app",
            "/release/target/candidates/0.4.0/ga/40050/signing-output/"
...
class ShellCleanupContractTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.preflight = self.root / "ga-preflight/40050"
        self.preflight.mkdir(parents=True, mode=0o700)
        self.frozen = self.root / "ga/40050"
    ...
    def test_frozen_submission_dispatch_derives_only_the_canonical_signed_app(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                context.staged_app,
                artifact_repository
                / "target/candidates/0.4.0/ga/40050/signing-output/signing-input/Clash for Mac.app",
            )
            self.assertEqual(
    ...
    def test_non_active_build_number_is_explicitly_rejected(self) -> None:
        for build_number in (
            ...
            with self.subTest(build_number=build_number):
                ...
                self.assertEqual(result.returncode, 2)
                self.assertIn("invalid choice", result.stderr)
                self.assertIn("choose from '40050'", result.stderr)
