...
class Fixture:
    def __init__(self) -> None:
        ...
        self.build = self.candidate / "ga/40051"
        ...
        self.signing_transformation_receipt = {
            ...
            "product": {"build_number": "40051", "version": "0.4.0"},
            "profiles": {
                "host": "c" * 64,
                "packet_tunnel": "d" * 64,
                "proxy_agent": "e" * 64,
...
class NotarizationTransactionSuccessTests(unittest.TestCase):
    ...
    def test_ga_uses_the_fixed_candidate_destinations(self) -> None:
        ...
        self.assertEqual(
            final_app,
            self.fixture.candidate / "ga/40051/signed/Clash for Mac.app",
        )
        self.assertEqual(
            context.native_products,
            (
                self.fixture.candidate
                / "ga/40051/signing-output/signed-native-products"
            ),
        )
        self.assertEqual(
            context.attempt_root,
            self.fixture.candidate / "ga/40051/transactions/app-notary",
        )
        manifest = json.loads(
        ...
        self.assertEqual(manifest["metadata"]["buildNumber"], "40051")
...
class NotarizationRecoveryTests(unittest.TestCase):
    ...
    def test_current_ga_fixture_continues_once_then_recovers_locally(self) -> None:
        ...
        self.assertEqual(fixture.context.build_number, "40051")
        self.assertEqual(
            fixture.context.archive_name,
            "Clash.for.Mac_0.4.0_40051_notary.zip",
        )
        ...
        self.assertEqual(
            first,
            fixture.candidate / "ga/40051/signed/Clash for Mac.app",
        )
        ...
        self.assertEqual(final_manifest["metadata"]["buildNumber"], "40051")
        self.assertEqual(
...
repo_root=/release
candidate_root=/release/target/candidates/0.4.0/ga/40051
...
"""
    ...
    def _notary_call(*mode: str) -> list[str]:
        return [
            ...
            *mode,
            "--native-products",
            "/release/target/candidates/0.4.0/ga/40051/signing-output/signed-native-products",
            "--notary-profile",
            "clashformac-notary",
            "--repository-commit",
            "a" * 40,
    ...
    def test_build_and_signing_resume_keep_full_signing_verification(self) -> None:
        staged = (
            "--staged-app",
            "/release/target/candidates/0.4.0/ga/40051/signing-output/"
...
class ShellCleanupContractTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.preflight = self.root / "ga-preflight/40051"
        self.preflight.mkdir(parents=True, mode=0o700)
        self.frozen = self.root / "ga/40051"
...
class NotarizationProxyEnvironmentTests(unittest.TestCase):
    def test_proxy_is_scoped_to_apple_requests_without_changing_parent_environment(self) -> None:
        command = ["/usr/bin/xcrun", "notarytool", "info", SUBMISSION_ID]
        with patch.dict(os.environ, {"CFW_NOTARY_PROXY": "http://127.0.0.1:7890"}, clear=True):
            with patch.object(transaction_module, "_run_bounded_process", return_value=CommandResult(0, "{}", "")) as run:
                transaction_module.production_command_runner(CommandRole.INFO, command, 5)
                ...
                run.reset_mock()
                transaction_module.production_command_runner(CommandRole.FINAL_VERIFY, command, 5)
                run.assert_called_once_with(command, 5)

    def test_invalid_proxy_never_starts_a_process_or_echoes_its_value(self) -> None:
        for proxy in ("", "http://remote.invalid:7890", "http://user:secret@127.0.0.1:7890",
                      "http://127.0.0.1", "http://127.0.0.1:65536", "http://127.0.0.1:7890/path",
                      "http://127.0.0.1:7890\n", "socks5://127.0.0.1:7890"):
            with self.subTest(proxy=proxy), patch.dict(os.environ, {"CFW_NOTARY_PROXY": proxy}, clear=True):
                with patch.object(transaction_module, "_run_bounded_process") as run:
                    with self.assertRaises(TransactionError) as failure:
                        transaction_module.production_command_runner(CommandRole.SUBMIT, ["notarytool"], 5)
                    self.assertEqual(failure.exception.code, "invalid_notary_proxy")
    ...
    def test_frozen_submission_dispatch_derives_only_the_canonical_signed_app(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                context.staged_app,
                artifact_repository
                / "target/candidates/0.4.0/ga/40051/signing-output/signing-input/Clash for Mac.app",
            )
            self.assertEqual(
    ...
    def test_non_active_build_number_is_explicitly_rejected(self) -> None:
        for build_number in (
            ...
            with self.subTest(build_number=build_number):
                ...
                self.assertEqual(result.returncode, 2)
                self.assertIn("invalid choice", result.stderr)
                self.assertIn("choose from '40051'", result.stderr)
