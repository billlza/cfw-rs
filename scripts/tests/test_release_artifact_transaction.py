...
def create_prepackage_stage(repository: Path) -> tuple[Path, dict[str, object]]:
    manifest = (
        repository
        / "target/candidates/0.4.0/ga/40059/prepackage/manifest.json"
    )
...
def create_signed_candidate(repository: Path, build_number: str = "40059") -> Path:
    app = (
        repository
...
class DmgFixture:
    def __init__(self) -> None:
        ...
        self.ga_root = self.repository / "target/candidates/0.4.0/ga/40059"
...
class DmgNotarizationTransactionTests(unittest.TestCase):
    ...
    def test_context_derives_only_the_fixed_ga_package_and_transaction_roots(self) -> None:
        self.assertEqual(
            ...
            / "target/candidates/0.4.0/ga/40059/packages/dmg/v0.4.0",
        )
        self.assertEqual(
            self.fixture.context.attempt_root,
            self.fixture.repository
            / "target/candidates/0.4.0/ga/40059/transactions/dmg-notary/v0.4.0",
        )
...
class UpdaterArtifactSetTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.package_root = (
            self.root / "target/candidates/0.4.0/ga/40059/packages"
        )
...
class ReleaseUploadGateTests(unittest.TestCase):
    ...
    def test_gate_rejects_legacy_unsealed_assets_at_the_package_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            package_root = (
                repository / "target/candidates/0.4.0/ga/40059/packages"
            )
    ...
    def test_shell_packagers_require_prepackage_and_fixed_ga_paths(self) -> None:
        ...
        for relative in ("make_dmg.sh", "make_updater_manifest.sh"):
            ...
            with self.subTest(script=relative):
                self.assertIn("target/candidates/0.4.0/ga/40059", source)
