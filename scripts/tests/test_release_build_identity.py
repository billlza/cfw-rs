...
class SignedPreviewIdentityTests(unittest.TestCase):
    ...
    def make_pair(
        ...
        *,
        attempt_id: str = "00000001",
        builds: tuple[str, str, str, str] = ("50011",) * 4,
        version: str = "0.5.0",
    ) -> tuple[Path, Path, Path]:
    ...
    def test_preview_identity_is_independent_and_exact(self) -> None:
        ...
        self.assertEqual(ids.SIGNED_PREVIEW_IDENTITY.build_number, "50011")
        for version, build in (("0.4.0", "50011"), ("0.5.0", "40073"), ("0.5.0", "50001"), ("0.5.0", "50002"), ("0.5.0", "50003"), ("0.5.0", "50004"), ("0.5.0", "50005"), ("0.5.0", "50006"), ("0.5.0", "50007"), ("0.5.0", "50008"), ("0.5.0", "50009"), ("0.5.0", "50010"), ("0.5.0", "50012"), ("0.5.0", "050011")):
        ...
        preflight = repository / "target/candidates/0.5.0/preview-preflight/50011"
        root = repository / "target/candidates/0.5.0/preview/50011"
    ...
    def test_preview_bundle_requires_explicit_version(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            app = self.make_app(Path(directory), ("50011",) * 4, version="0.5.0")
            self.assertEqual(
                bundle_build_identity(app, expected_product_version="0.5.0"),
                BundleBuildIdentity("0.5.0", "50011"),
            )
            with self.assertRaises(BuildIdentityError):
                bundle_build_identity(app)
            with self.assertRaises(BuildIdentityError):
    ...
    def test_all_preview_contexts_accept_exact_pairs(self) -> None:
        for context in self.contexts:
            with self.subTest(context=context), tempfile.TemporaryDirectory() as directory:
                ...
                self.assertEqual(value.build_identity, BundleBuildIdentity("0.5.0", "50011"))
    ...
    def test_preview_rejects_uniform_wrong_or_noncanonical_builds(self) -> None:
        for build in ("40073", "40000", "50001", "50002", "50003", "50004", "50005", "50006", "50007", "50008", "50009", "50010", "50012", "050011", "50011\n", "0", "+50011", "9223372036854775808"):
    ...
    def test_preview_native_output_has_exact_unallocated_root(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            repository = Path(directory).resolve()
            native = ids.preview_native_products_root(repository)
            self.assertEqual(candidate_native_products_output(repository, str(native), "50011"), native)
            derived = native.parent / "xcode-derived-data"
            self.assertEqual(candidate_native_derived_data_output(repository, str(native), str(derived), "50011"), derived)
            self.assertFalse((repository / "target").exists())
            for build, output in (
                ("40073", native), ("40000", native), ("050011", native), ("50008", native), ("50009", native), ("50010", native), ("50012", native),
                ("50011", ga_pre_sign_native_products_root(repository)),
                ("50011", repository / "target/candidates/0.5.0/unsigned/native-products"),
                ("50011", repository / "target/candidates/0.5.0/ga-preflight/50011/native-products"),
                ("50011", native.parent / "nested/native-products"),
            ):
            ...
            with self.assertRaises(BuildIdentityError):
                candidate_native_derived_data_output(repository, str(native), str(ga_preflight_root(repository) / "xcode-derived-data"), "50011")
    ...
    def test_preview_pre_sign_only_accepts_two_exact_host_paths(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            built = self.make_app(preflight / "cargo/release/bundle/macos", ("50011",) * 4, version="0.5.0")
            ...
            for root in (preflight / "other", release_build_identity.preview_signed_root(repository), repository / "outside"):
                app = self.make_app(root, ("50011",) * 4, version="0.5.0")
    ...
    def test_preview_and_ga_contexts_cannot_be_crossed(self) -> None:
        for context in self.contexts:
            with self.subTest(context=context), tempfile.TemporaryDirectory() as directory:
                ...
                ga_app = self.make_app(ga_signed_root(repository), ("50011",) * 4, version="0.5.0")
    ...
    def test_preview_paths_reject_aliases_and_symlink_ancestors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(BuildIdentityError):
                candidate_bundle_verification_paths(repository, app, native, context)
            with self.assertRaises(BuildIdentityError):
                candidate_native_products_output(repository, str(native), "50011")
    ...
    def test_preview_canonical_allows_verified_copies_not_preflight_or_ga_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            copy = self.make_app(repository / "target/notarization/preview-copy", ("50011",) * 4, version="0.5.0")
            ...
            false_ga = self.make_app(repository / "target/candidates/0.5.0/ga/50011/signed", ("50011",) * 4, version="0.5.0")
