...
class ReleaseBuildIdentityTests(unittest.TestCase):
    ...
    def test_ga_paths_are_fixed_to_the_single_active_identity(self) -> None:
        ...
        self.assertEqual(
            ...
            Path("/repo/target/candidates/0.4.0/ga-preflight/40054"),
        )
        self.assertEqual(
            ga_root(repository),
            Path("/repo/target/candidates/0.4.0/ga/40054"),
        )
        self.assertEqual(
            ga_pre_sign_native_products_root(repository),
            Path(
                "/repo/target/candidates/0.4.0/ga-preflight/40054/native-products"
            ),
        )
        self.assertEqual(
            ga_signed_root(repository),
            Path("/repo/target/candidates/0.4.0/ga/40054/signed"),
        )
        self.assertEqual(
            ga_signed_native_products_root(repository),
            Path(
                "/repo/target/candidates/0.4.0/ga/40054/signing-output/signed-native-products"
            ),
        )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            rejected = (
                (
                    "40054",
                    repository
                    / "target/candidates/0.4.0/validation/40054/native-products",
                ),
                (
                    "40054",
                    repository
                    / "target/candidates/0.4.0/release-build/40054/native-products",
                ),
                (
                    "40030",
                    repository
            ...
            with self.assertRaises(BuildIdentityError):
                candidate_native_products_output(
                    repository,
                    str(repository)
                    + "/target/candidates/0.4.0/ga-preflight//40054/native-products",
                    "40054",
                )
    ...
    def test_bundle_context_accepts_exact_private_work_and_publish_ready(self) -> None:
        for context in (
            ...
            with self.subTest(context=context), tempfile.TemporaryDirectory() as directory:
                ...
                self.assertEqual(paths.build_identity.build_version, "40054")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    ...
                    return_value=BundleBuildIdentity("0.4.0", "40054"),
                ),
                patch(
                    "scripts.release_build_identity.os.geteuid",
                    return_value=os.geteuid() + 1,
