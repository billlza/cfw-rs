...
class TauriHostSkeletonRunnerTests(unittest.TestCase):
    ...
    def test_unsigned_preview_mode_enables_real_components_and_refuses_wrong_build_before_tauri(self) -> None:
        ...
        for build in ("40000", "40073", "50013", "50014", "50015", "50016", "050000", "+50000"):
    ...
    def test_preview_build_is_explicit_and_does_not_enable_rejected_dashboard(self) -> None:
        self.write_config({"version": "0.5.0", "bundle": {"macOS": {}}})
        override = json.dumps({"bundle": {"macOS": {"bundleVersion": "50016"}}})
        completed = self.run_contract(preview=True, override=override, environment_updates={"CFW_BUILD_NUMBER": "50016"})
        self.assertEqual(completed.returncode, 0, completed.stderr.decode())
        ...
        for environment, config in [
            ...
            ({"CFW_BUILD_NUMBER": "50016"}, self.override),
            ({"CFW_BUILD_NUMBER": "50016", "APPLE_SIGNING_IDENTITY": "unexpected"}, override),
        ]:
        ...
        denied = self.run_contract(preview=True, override=override, environment_updates={"CFW_BUILD_NUMBER": "50016"})
        self.assertNotEqual(denied.returncode, 0)
        self.assertNotIn(b"[build]", denied.stdout)
