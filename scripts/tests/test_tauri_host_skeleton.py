...
class TauriHostSkeletonRunnerTests(unittest.TestCase):
    ...
    def test_preview_build_is_explicit_and_does_not_enable_rejected_dashboard(self) -> None:
        self.write_config({"version": "0.5.0", "bundle": {"macOS": {}}})
        override = json.dumps({"bundle": {"macOS": {"bundleVersion": "50011"}}})
        completed = self.run_contract(preview=True, override=override, environment_updates={"CFW_BUILD_NUMBER": "50011"})
        self.assertEqual(completed.returncode, 0, completed.stderr.decode())
        ...
        for environment, config in [
            ...
            ({"CFW_BUILD_NUMBER": "50012"}, override),
            ({"CFW_BUILD_NUMBER": None}, override),
            ({"CFW_BUILD_NUMBER": "50011"}, self.override),
            ({"CFW_BUILD_NUMBER": "50011", "APPLE_SIGNING_IDENTITY": "unexpected"}, override),
        ]:
        ...
        denied = self.run_contract(preview=True, override=override, environment_updates={"CFW_BUILD_NUMBER": "50011"})
        self.assertNotEqual(denied.returncode, 0)
        self.assertNotIn(b"[build]", denied.stdout)
